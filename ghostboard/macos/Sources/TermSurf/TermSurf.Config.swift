...
extension TermSurf {
    /// Maps to a `termsurf_config_t` and the various operations on that.
    class Config: ObservableObject {
        // The underlying C pointer to the TermSurf config structure. This
        // should never be accessed directly. Any operations on this should
        // be called from the functions on this or another class.
        private(set) var config: termsurf_config_t? = nil {
            didSet {
                // Free the old value whenever we change
                guard let old = oldValue else { return }
                termsurf_config_free(old)
            }
        }
        ...
        var errors: [String] {
            ...
            var diags: [String] = [];
            let diagsCount = termsurf_config_diagnostics_count(cfg)
            for i in 0..<diagsCount {
                let diag = termsurf_config_get_diagnostic(cfg, UInt32(i))
        ...
        convenience init(clone config: termsurf_config_t) {
            self.init(config: termsurf_config_clone(config))
        }
        ...
        static private func loadConfig(at path: String?, finalize: Bool) -> termsurf_config_t? {
            // Initialize the global configuration.
            guard let cfg = termsurf_config_new() else {
                logger.critical("termsurf_config_new failed")
                return nil
            }
...
#if os(macOS)
            if let path {
                termsurf_config_load_file(cfg, path)
            } else {
                termsurf_config_load_default_files(cfg)
            }

            // We only load CLI args when not running in Xcode because in Xcode we
            // pass some special parameters to control the debugger.
            if !isRunningInXcode() {
                termsurf_config_load_cli_args(cfg);
            }

            termsurf_config_load_recursive_files(cfg);
#endif
        ...
        /// in the TermSurf configuration. For example `keybind = cmd+q=quit` in TermSurf
        ...
        func keyboardShortcut(for action: String) -> KeyboardShortcut? {
            ...
            let trigger = termsurf_config_trigger(cfg, action, UInt(action.lengthOfBytes(using: .utf8)))
            return TermSurf.keyboardShortcut(for: trigger)
        }
#endif

        // MARK: - Configuration Values

        /// For all of the configuration values below, see the associated TermSurf documentation for
        ...
        var bellFeatures: BellFeatures {
            ...
            guard termsurf_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .init() }
        ...
        var splitPreserveZoom: SplitPreserveZoom {
            ...
            guard termsurf_config_get(config, &v, key, UInt(key.count)) else { return .init() }
            return .init(rawValue: v)
        }
        ...
        var initialWindow: Bool {
            ...
            _ = termsurf_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8)))
            return v
        }
        ...
        var title: String? {
            ...
            let key = "title"
            guard termsurf_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return nil }
        ...
        var windowSaveState: String {
            ...
            guard termsurf_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return "" }
        ...
        var windowPositionX: Int16? {
            ...
            return termsurf_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) ? v : nil
        }
        ...
        var windowDecorations: Bool {
            ...
            guard termsurf_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return defaultValue }
        ...
        var macosHidden: MacHidden {
            ...
            guard termsurf_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .never }
        ...
        var backgroundColor: Color {
            ...
            if (!termsurf_config_get(config, &color, bg_key, UInt(bg_key.lengthOfBytes(using: .utf8)))) {
#if os(macOS)
...
#error("unsupported")
        ...
        var unfocusedSplitOpacity: Double {
            ...
            _ = termsurf_config_get(config, &opacity, key, UInt(key.lengthOfBytes(using: .utf8)))
            return 1 - opacity
        }
        ...
        var unfocusedSplitFill: Color {
            ...
            if (!termsurf_config_get(config, &color, key, UInt(key.lengthOfBytes(using: .utf8)))) {
                let bg_key = "background"
                _ = termsurf_config_get(config, &color, bg_key, UInt(bg_key.lengthOfBytes(using: .utf8)));
            }
        ...
        var quickTerminalPosition: QuickTerminalPosition {
            ...
            guard termsurf_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .top }
        ...
        var quickTerminalScreen: QuickTerminalScreen {
            ...
            guard termsurf_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .main }
            guard let ptr = v else { return .main }
            let str = String(cString: ptr)
            return QuickTerminalScreen(fromTermSurfConfig: str) ?? .main
        }
        ...
        var quickTerminalSpaceBehavior: QuickTerminalSpaceBehavior {
            ...
            guard termsurf_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .move }
            guard let ptr = v else { return .move }
            let str = String(cString: ptr)
            return QuickTerminalSpaceBehavior(fromTermSurfConfig: str) ?? .move
        }

        var quickTerminalSize: QuickTerminalSize {
            guard let config = self.config else { return QuickTerminalSize() }
            var v = termsurf_config_quick_terminal_size_s()
            let key = "quick-terminal-size"
            guard termsurf_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return QuickTerminalSize() }
        ...
        var resizeOverlay: ResizeOverlay {
            ...
            guard termsurf_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .after_first }
        ...
        var commandPaletteEntries: [TermSurf.Command] {
            guard let config = self.config else { return [] }
            var v: termsurf_config_command_list_s = .init()
            let key = "command-palette-entry"
            guard termsurf_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return [] }
            guard v.len > 0 else { return [] }
            let buffer = UnsafeBufferPointer(start: v.commands, count: v.len)
            return buffer.map { TermSurf.Command(cValue: $0) }
        }
    }
}
