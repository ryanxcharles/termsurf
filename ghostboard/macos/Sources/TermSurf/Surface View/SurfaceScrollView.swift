...
/// Wraps a TermSurf surface view in an NSScrollView to provide native macOS scrollbar support.
///
/// ## Coordinate System
...
/// - `surfaceView`: The actual TermSurf renderer, positioned to fill the visible rect
class SurfaceScrollView: NSView {
    private let scrollView: NSScrollView
    private let documentView: NSView
    private let surfaceView: TermSurf.SurfaceView
    ...
    init(contentSize: CGSize, surfaceView: TermSurf.SurfaceView) {
        ...
        // so that our primary TermSurf renderer only needs to render the viewport.
        ...
        // Listen for scrollbar updates from TermSurf
        observers.append(NotificationCenter.default.addObserver(
            forName: .termsurfDidUpdateScrollbar,
            object: surfaceView,
            queue: .main
        ) { [weak self] notification in
    ...
    private func handleScrollbarUpdate(_ notification: Notification) {
        guard let scrollbar = notification.userInfo?[SwiftUI.Notification.Name.ScrollbarKey] as? TermSurf.Action.Scrollbar else {
            return
        }
