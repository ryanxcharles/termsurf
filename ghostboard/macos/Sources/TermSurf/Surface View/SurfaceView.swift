...
extension TermSurf {
    ...
    struct Terminal: View {
        @EnvironmentObject private var termsurf: TermSurf.App

        var body: some View {
            if let app = self.termsurf.app {
    ...
    /// Yields a SurfaceView for a termsurf app that can then be used however you want.
    struct SurfaceForApp<Content: View>: View {
        let content: ((SurfaceView) -> Content)

        @StateObject private var surfaceView: SurfaceView

        init(_ app: termsurf_app_t, @ViewBuilder content: @escaping ((SurfaceView) -> Content)) {
    ...
    struct SurfaceWrapper: View {
        ...
        var body: some View {
            ...
            ZStack {
                ...
                GeometryReader { geo in
                    ...
                    if let surfaceSize = surfaceView.surfaceSize {
                        SurfaceResizeOverlay(
                            geoSize: geo.size,
                            size: surfaceSize,
                            overlay: termsurf.config.resizeOverlay,
                            position: termsurf.config.resizeOverlayPosition,
                            duration: termsurf.config.resizeOverlayDuration,
                            focusInstant: surfaceView.focusInstant)

                    }
                }
                .termsurfSurfaceView(surfaceView)
                ...
                if (termsurf.config.secureInputIndication &&
                    secureInput.enabled &&
                    surfaceFocus &&
                    windowFocus) {
                    SecureInputOverlay()
                ...
                if let searchState = surfaceView.searchState {
                    SurfaceSearchOverlay(
                        ...
                        onClose: {
#if canImport(AppKit)
                            TermSurf.moveFocus(to: surfaceView)
#endif
                ...
                if (termsurf.config.bellFeatures.contains(.border)) {
                ...
                if (!surfaceView.healthy) {
                    Rectangle().fill(termsurf.config.backgroundColor)
    ...
    struct SurfaceResizeOverlay: View {
        let geoSize: CGSize
        let size: termsurf_surface_size_s
        let overlay: TermSurf.Config.ResizeOverlay
        let position: TermSurf.Config.ResizeOverlayPosition
        let duration: UInt
    ...
    struct SurfaceSearchOverlay: View {
        ...
        var body: some View {
            GeometryReader { geo in
                ...
                .onReceive(NotificationCenter.default.publisher(for: .termsurfSearchFocus)) { notification in
    ...
    /// A surface is terminology in TermSurf for a terminal surface, or a place where a terminal is actually drawn
    ...
    /// libtermsurf, usually from the TermSurf configuration.
    struct SurfaceConfiguration {
        ...
        var context: termsurf_surface_context_e = TERMSURF_SURFACE_CONTEXT_WINDOW

        init() {}

        init(from config: termsurf_surface_config_s) {
        ...
        /// Provides a C-compatible termsurf configuration within a closure. The configuration
        /// and all its string pointers are only valid within the closure.
        func withCValue<T>(view: SurfaceView, _ body: (inout termsurf_surface_config_s) throws -> T) rethrows -> T {
            var config = termsurf_surface_config_new()
            config.userdata = Unmanaged.passUnretained(view).toOpaque()
#if os(macOS)
            config.platform_tag = TERMSURF_PLATFORM_MACOS
            config.platform = termsurf_platform_u(macos: termsurf_platform_macos_s(
                nsview: Unmanaged.passUnretained(view).toOpaque()
            ))
            config.scale_factor = NSScreen.main!.backingScaleFactor
#elseif os(iOS)
            config.platform_tag = TERMSURF_PLATFORM_IOS
            config.platform = termsurf_platform_u(ios: termsurf_platform_ios_s(
            ...
            // libtermsurf APIs when a UIView is attached to a window/scene. TODO.
                    ...
                    return try initialInput.withCString { cInput in
                        ...
                        return try keys.withCStrings { keyCStrings in
                            return try values.withCStrings { valueCStrings in
                                // Create array of termsurf_env_var_s
                                var envVars = Array<termsurf_env_var_s>()
                                envVars.reserveCapacity(environmentVariables.count)
                                for i in 0..<environmentVariables.count {
                                    envVars.append(termsurf_env_var_s(
                                        key: keyCStrings[i],
...
private struct TermSurfSurfaceViewKey: EnvironmentKey {
    static let defaultValue: TermSurf.SurfaceView? = nil
}

extension EnvironmentValues {
    var termsurfSurfaceView: TermSurf.SurfaceView? {
        get { self[TermSurfSurfaceViewKey.self] }
        set { self[TermSurfSurfaceViewKey.self] = newValue }
    }
}

extension View {
    func termsurfSurfaceView(_ surfaceView: TermSurf.SurfaceView?) -> some View {
        environment(\.termsurfSurfaceView, surfaceView)
    }
}
...
extension FocusedValues {
    var termsurfSurfaceView: TermSurf.SurfaceView? {
        get { self[FocusedTermSurfSurface.self] }
        set { self[FocusedTermSurfSurface.self] = newValue }
    }

    struct FocusedTermSurfSurface: FocusedValueKey {
        typealias Value = TermSurf.SurfaceView
    }

    var termsurfSurfacePwd: String? {
        get { self[FocusedTermSurfSurfacePwd.self] }
        set { self[FocusedTermSurfSurfacePwd.self] = newValue }
    }

    struct FocusedTermSurfSurfacePwd: FocusedValueKey {
        typealias Value = String
    }

    var termsurfSurfaceCellSize: OSSize? {
        get { self[FocusedTermSurfSurfaceCellSize.self] }
        set { self[FocusedTermSurfSurfaceCellSize.self] = newValue }
    }

    struct FocusedTermSurfSurfaceCellSize: FocusedValueKey {
...
extension TermSurf.SurfaceView {
    class SearchState: ObservableObject {
        ...
        init(from startSearch: TermSurf.Action.StartSearch) {
