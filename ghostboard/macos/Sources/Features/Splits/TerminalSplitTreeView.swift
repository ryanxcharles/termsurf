...
enum TerminalSplitOperation {
    ...
    struct Resize {
        let node: SplitTree<TermSurf.SurfaceView>.Node
        let ratio: Double
    }
    
    struct Drop {
        /// The surface being dragged.
        let payload: TermSurf.SurfaceView
        
        /// The surface it was dragged onto
        let destination: TermSurf.SurfaceView
...
struct TerminalSplitTreeView: View {
    let tree: SplitTree<TermSurf.SurfaceView>
    ...
    var body: some View {
        ...
        case .split(let split):
            ...
            SplitView(
                ...
                dividerColor: termsurf.config.splitDividerColor,
...
fileprivate struct TerminalSplitLeaf: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            .background {
                ...
                if !isSelfDragging {
                    Color.clear
                        .onDrop(of: [.termsurfSurfaceId], delegate: SplitDropDelegate(
                            dropState: $dropState,
            ...
            .onPreferenceChange(TermSurf.DraggingSurfaceKey.self) { value in
    ...
    private struct SplitDropDelegate: DropDelegate {
        ...
        let viewSize: CGSize
        let destinationSurface: TermSurf.SurfaceView
        let action: (TerminalSplitOperation) -> Void
        
        func validateDrop(info: DropInfo) -> Bool {
            info.hasItemsConforming(to: [.termsurfSurfaceId])
        }
        ...
        func performDrop(info: DropInfo) -> Bool {
            ...
            let providers = info.itemProviders(for: [.termsurfSurfaceId])
            guard let provider = providers.first else { return false }
            
            // Capture action before the async closure
            _ = provider.loadTransferable(type: TermSurf.SurfaceView.self) { [weak destinationSurface] result in
                switch result {
