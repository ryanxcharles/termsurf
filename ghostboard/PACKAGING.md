# Packaging TermSurf for Distribution

TermSurf relies on downstream package maintainers to distribute TermSurf to
end-users. This document provides guidance to package maintainers on how to
package TermSurf for distribution.

> [!IMPORTANT]
>
> This document is only accurate for the TermSurf source alongside it.
> **Do not use this document for older or newer versions of TermSurf!** If
> you are reading this document in a different version of TermSurf, please
...
```
https://release.files.ghostty.org/VERSION/termsurf-VERSION.tar.gz
https://release.files.ghostty.org/VERSION/termsurf-VERSION.tar.gz.minisig
```
...
Use the `termsurf-source.tar.gz` asset and _not the GitHub auto-generated
...
> contain some preprocessed files that allow building TermSurf with less
> dependencies. If you are building TermSurf from a Git checkout, the
...
> from a Git checkout.
>
> For everyone except TermSurf developers, please use the source tarballs.
> We generate tip source tarballs for users following the development
> branch.

## Zig Version

[Zig](https://ziglang.org) is required to build TermSurf. Prior to Zig 1.0,
Zig releases often have breaking changes. TermSurf requires specific Zig versions
depending on the TermSurf version in order to build. To make things easier for
package maintainers, TermSurf always uses some _released_ version of Zig.

To find the version of Zig required to build TermSurf, check the `required_zig`
...
For example, at the time of writing this document, TermSurf requires Zig 0.14.0.

## Building TermSurf

The following is a standard example of how to build TermSurf _for system
packages_. This is not the recommended way to build TermSurf for your
...
2. Next, we build TermSurf. This step requires no internet access:

```sh
DESTDIR=/tmp/termsurf \
zig build \
  --prefix /usr \
...
and install TermSurf to `/tmp/termsurf` with the prefix `/usr` (i.e. the
binary will be at `/tmp/termsurf/usr/bin/termsurf`). This style is common
for system packages which separate a build and install step, since the
install step can then be done with a `mv` or `cp` command (from `/tmp/termsurf`
to wherever the package manager expects it).

### Build Options

TermSurf uses the Zig build system. You can see all available build options by
