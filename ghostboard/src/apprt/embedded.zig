//! Application runtime for the embedded version of TermSurf. The embedded
//! version is when TermSurf is embedded within a parent host application,
//! rather than owning the application lifecycle itself. This is used for
//! example for the macOS build of TermSurf so that we can use a native
...
pub const App = struct {
    ...
    ///
    /// C type: termsurf_runtime_config_s
    ...
    /// This is the key event sent for termsurf_surface_key and
    /// termsurf_app_key.
    pub const KeyEvent = struct {
        ...
        /// Convert a libtermsurf key event into a core key event.
    ...
    /// Send the given IPC to a running TermSurf. Returns `true` if the action was
    /// able to be performed, `false` otherwise.
    ///
    /// Note that this is a static function. Since this is called from a CLI app (or
    /// some other process that is not TermSurf) there is no full-featured apprt App
    /// to use.
    pub fn performIpc(
        _: Allocator,
        _: apprt.ipc.Target,
...
/// Platform-specific configuration for libtermsurf.
pub const Platform = union(PlatformTag) {
    macos: MacOS,
    ios: IOS,

    // If our build target for libtermsurf is not darwin then we do
...
pub const Surface = struct {
    ...
    pub const Options = extern struct {
        ...
        /// despite TermSurf allowing directly executed commands via config.
    ...
    pub fn defaultTermioEnv(self: *const Surface) !std.process.EnvMap {
        ...
        if (comptime builtin.target.os.tag.isDarwin()) {
            ...
            // Remove this so that running `termsurf` within TermSurf works.
            env.remove("TERMSURF_MAC_LAUNCH_SOURCE");

            // If we were launched from the desktop then we want to
            // remove the LANGUAGE env var so that we don't inherit
            // our translation settings for TermSurf. If we aren't from
...
pub const Inspector = struct {
    ...
    pub fn init(surface: *Surface) !Inspector {
        ...
        io.BackendPlatformName = "termsurf_embedded";
...
pub const CAPI = struct {
    ...
    export fn termsurf_app_tick(v: *App) void {
    ...
    export fn termsurf_app_userdata(v: *App) ?*anyopaque {
        return v.opts.userdata;
    }

    export fn termsurf_app_free(v: *App) void {
    ...
    export fn termsurf_app_set_focus(
        app: *App,
        focused: bool,
    ) void {
    ...
    export fn termsurf_app_key_is_binding(
        app: *App,
        event: KeyEvent,
    ) bool {
    ...
    export fn termsurf_app_keyboard_changed(v: *App) void {
    ...
    export fn termsurf_app_open_config(v: *App) void {
    ...
    export fn termsurf_app_update_config(
        v: *App,
        config: *const Config,
    ) void {
    ...
    export fn termsurf_app_needs_confirm_quit(v: *App) bool {
        return v.core_app.needsConfirmQuit();
    }

    /// Returns true if the app has global keybinds.
    export fn termsurf_app_has_global_keybinds(v: *App) bool {
        return v.hasGlobalKeybinds();
    }

    /// Update the color scheme of the app.
    export fn termsurf_app_set_color_scheme(v: *App, scheme_raw: c_int) void {
        const scheme = std.meta.intToEnum(apprt.ColorScheme, scheme_raw) catch {
            log.warn(
                "invalid color scheme to termsurf_surface_set_color_scheme value={}",
                .{scheme_raw},
            );
            return;
        };
    ...
    export fn termsurf_surface_config_new() apprt.Surface.Options {
        return .{};
    }

    /// Create a new surface as part of an app.
    export fn termsurf_surface_new(
        app: *App,
    ...
    export fn termsurf_surface_free(ptr: *Surface) void {
        ptr.app.closeSurface(ptr);
    }

    /// Returns the userdata associated with the surface.
    export fn termsurf_surface_userdata(surface: *Surface) ?*anyopaque {
        return surface.userdata;
    }

    /// Returns the app associated with a surface.
    export fn termsurf_surface_app(surface: *Surface) *App {
        return surface.app;
    }

    /// Returns the config to use for surfaces that inherit from this one.
    export fn termsurf_surface_inherited_config(
        surface: *Surface,
    ...
    export fn termsurf_surface_update_config(
        surface: *Surface,
        config: *const Config,
    ) void {
    ...
    export fn termsurf_surface_needs_confirm_quit(surface: *Surface) bool {
        return surface.core_surface.needsConfirmQuit();
    }

    /// Returns true if the surface process has exited.
    export fn termsurf_surface_process_exited(surface: *Surface) bool {
        return surface.core_surface.child_exited;
    }

    /// Returns true if the surface has a selection.
    export fn termsurf_surface_has_selection(surface: *Surface) bool {
        return surface.core_surface.hasSelection();
    }

    /// Same as termsurf_surface_read_text but reads from the user selection,
    /// if any.
    export fn termsurf_surface_read_selection(
        surface: *Surface,
        result: *Text,
    ) bool {
    ...
    export fn termsurf_surface_read_text(
        surface: *Surface,
        sel: Selection,
        result: *Text,
    ) bool {
    ...
    export fn termsurf_surface_free_text(ptr: *Text) void {
        ptr.deinit();
    }

    /// Tell the surface that it needs to schedule a render
    export fn termsurf_surface_refresh(surface: *Surface) void {
        surface.refresh();
    }

    /// Tell the surface that it needs to schedule a render
    /// call as soon as possible (NOW if possible).
    export fn termsurf_surface_draw(surface: *Surface) void {
        surface.draw();
    }

    /// Update the size of a surface. This will trigger resize notifications
    /// to the pty and the renderer.
    export fn termsurf_surface_set_size(surface: *Surface, w: u32, h: u32) void {
        surface.updateSize(w, h);
    }

    /// Return the size information a surface has.
    export fn termsurf_surface_size(surface: *Surface) SurfaceSize {
    ...
    export fn termsurf_surface_set_color_scheme(surface: *Surface, scheme_raw: c_int) void {
        const scheme = std.meta.intToEnum(apprt.ColorScheme, scheme_raw) catch {
            log.warn(
                "invalid color scheme to termsurf_surface_set_color_scheme value={}",
                .{scheme_raw},
    ...
    export fn termsurf_surface_set_content_scale(surface: *Surface, x: f64, y: f64) void {
        surface.updateContentScale(x, y);
    }

    /// Update the focused state of a surface.
    export fn termsurf_surface_set_focus(surface: *Surface, focused: bool) void {
        surface.focusCallback(focused);
    }

    /// Notify XPC of pane focus change for Chromium focus lifecycle (Issue 606).
    export fn termsurf_surface_pane_focus_changed(surface: *Surface, focused: bool) void {
        surface.core_surface.paneFocusChanged(focused);
    }

    /// Update the occlusion state of a surface.
    export fn termsurf_surface_set_occlusion(surface: *Surface, visible: bool) void {
    ...
    export fn termsurf_surface_key_translation_mods(
        surface: *Surface,
        mods_raw: c_int,
    ) c_int {
    ...
    /// being consumed by TermSurf bindings or the macOS menu system.
    /// (Issue 609 Experiment 2.)
    export fn termsurf_surface_is_overlay_forwarding(
        surface: *Surface,
    ) bool {
    ...
    /// key and char events.
    export fn termsurf_surface_key(
        surface: *Surface,
        event: KeyEvent,
    ) bool {
    ...
    /// is important because things like trigger sequences are only
    /// valid until the next key event.
    export fn termsurf_surface_key_is_binding(
        surface: *Surface,
        event: KeyEvent,
    ...
    export fn termsurf_surface_text(
        surface: *Surface,
        ptr: [*]const u8,
        len: usize,
    ) void {
    ...
    export fn termsurf_surface_preedit(
        surface: *Surface,
        ptr: [*]const u8,
        len: usize,
    ) void {
    ...
    /// enabled.
    export fn termsurf_surface_mouse_captured(surface: *Surface) bool {
        return surface.core_surface.mouseCaptured();
    }

    /// Tell the surface that it needs to schedule a render
    export fn termsurf_surface_mouse_button(
        surface: *Surface,
        action: input.MouseButtonState,
        button: input.MouseButton,
        mods: c_int,
    ...
    export fn termsurf_surface_mouse_pos(
        surface: *Surface,
        x: f64,
        y: f64,
        mods: c_int,
    ...
    export fn termsurf_surface_mouse_scroll(
        surface: *Surface,
        x: f64,
        y: f64,
        scroll_mods: c_int,
    ...
    /// TermSurf's termsurf_ namespace.
    ...
    export fn termsurf_surface_mouse_pressure(
        surface: *Surface,
        stage_raw: u32,
        pressure: f64,
    ) void {
    ...
    export fn termsurf_surface_ime_point(
        surface: *Surface,
        x: *f64,
        y: *f64,
        width: *f64,
    ...
    export fn termsurf_surface_request_close(ptr: *Surface) void {
        ptr.core_surface.close();
    }

    /// Request that the surface split in the given direction.
    export fn termsurf_surface_split(ptr: *Surface, direction: apprt.action.SplitDirection) void {
    ...
    export fn termsurf_surface_split_resize(
        ptr: *Surface,
    ...
    export fn termsurf_surface_split_equalize(ptr: *Surface) void {
        _ = ptr.app.performAction(
            .{ .surface = &ptr.core_surface },
            .equalize_splits,
            {},
    ...
    export fn termsurf_surface_binding_action(
        ptr: *Surface,
        action_ptr: [*]const u8,
        action_len: usize,
    ) bool {
    ...
    export fn termsurf_surface_complete_clipboard_request(
        ptr: *Surface,
        str: [*:0]const u8,
        state: *apprt.ClipboardRequest,
        confirmed: bool,
    ...
    export fn termsurf_surface_inspector(ptr: *Surface) ?*Inspector {
    ...
    export fn termsurf_inspector_free(ptr: *Surface) void {
        ptr.freeInspector();
    }

    export fn termsurf_inspector_set_size(ptr: *Inspector, w: u32, h: u32) void {
        ptr.updateSize(w, h);
    }

    export fn termsurf_inspector_set_content_scale(ptr: *Inspector, x: f64, y: f64) void {
        ptr.updateContentScale(x, y);
    }

    export fn termsurf_inspector_mouse_button(
        ptr: *Inspector,
        action: input.MouseButtonState,
        button: input.MouseButton,
        mods: c_int,
    ...
    export fn termsurf_inspector_mouse_pos(ptr: *Inspector, x: f64, y: f64) void {
        ptr.cursorPosCallback(x, y);
    }

    export fn termsurf_inspector_mouse_scroll(
        ptr: *Inspector,
        x: f64,
        y: f64,
        scroll_mods: c_int,
    ...
    export fn termsurf_inspector_key(
        ptr: *Inspector,
        action: input.Action,
        key: input.Key,
        c_mods: c_int,
    ...
    export fn termsurf_inspector_text(
        ptr: *Inspector,
        str: [*:0]const u8,
    ) void {
    ...
    export fn termsurf_inspector_set_focus(ptr: *Inspector, focused: bool) void {
    ...
    export fn termsurf_set_window_background_blur(
        app: *App,
        window: *anyopaque,
    ) void {
    ...
    /// See termsurf_set_window_background_blur
    ...
    // Darwin-only C APIs.
    const Darwin = struct {
        export fn termsurf_surface_set_display_id(ptr: *Surface, display_id: u32) void {
        ...
        export fn termsurf_surface_quicklook_font(ptr: *Surface) ?*anyopaque {
        ...
        export fn termsurf_surface_quicklook_word(
            ptr: *Surface,
            result: *Text,
        ) bool {
            const surface = &ptr.core_surface;
        ...
        export fn termsurf_inspector_metal_init(ptr: *Inspector, device: objc.c.id) bool {
            return ptr.initMetal(.fromId(device));
        }

        export fn termsurf_inspector_metal_render(
            ptr: *Inspector,
        ...
        export fn termsurf_inspector_metal_shutdown(ptr: *Inspector) void {
            if (ptr.backend) |v| {
                v.deinit();
                ptr.backend = null;
            }
