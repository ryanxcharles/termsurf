//! The main entrypoint for the `termsurf` application. This also serves
//! as the process initialization code for the `libtermsurf` library.
...
const TermSurf = @import("main_c.zig").TermSurf;
...
pub fn main() !MainReturn {
    ...
    if (comptime build_config.app_runtime == .none) {
        ...
        try stdout.print(
            ...
            \\(i.e. "TermSurf Ghostboard.app" on macOS). This CLI can be used to perform various
            \\actions such as inspecting the version, listing fonts, etc.
            \\
            \\On macOS, the terminal can also be launched using `open -na "TermSurf Ghostboard.app"`,
            \\or `open -na "TermSurf Ghostboard.app" --args --foo=bar --baz=qux` to pass arguments.
            \\
...
fn logFn(
    ...
    //
    //   sudo log stream --level debug --predicate 'subsystem=="com.termsurf"'
    //
...
pub const std_options: std.Options = .{
    ...
    // via TERMSURF_LOG because our debug logs are very expensive to
