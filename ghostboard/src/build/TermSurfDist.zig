const TermSurfDist = @This();

const std = @import("std");
const Config = @import("Config.zig");
const SharedDeps = @import("SharedDeps.zig");
const TermSurfFrameData = @import("TermSurfFrameData.zig");
...
pub fn init(b: *std.Build, cfg: *const Config) !TermSurfDist {
    ...
    {
        const framedata = TermSurfFrameData.distResources(b);
    ...
    // embed the TermSurf version in the tarball
    {
        ...
        git_archive.addArg(b.fmt("--prefix=termsurf-{f}/", .{
            cfg.version,
        }));
    ...
    for (resources.items) |resource| {
        ...
        // --add-file uses the most recent --prefix to determine the path
        // in the archive to copy the file (the directory only).
        git_archive.addArg(b.fmt("--prefix=termsurf-{f}/{s}/", .{
            cfg.version,
    ...
    git_archive.addArgs(&.{
        ...
        b.fmt("--prefix=termsurf-{f}/", .{cfg.version}),
        "-o",
    });
    ...
    const install = b.addInstallFile(
        output,
        b.fmt("dist/termsurf-{f}.tar.gz", .{cfg.version}),
    );
    ...
    // This is the root TermSurf source dir of the extracted source tarball.
    // i.e. this is way `build.zig` is.
    const extract_dir = check
        .addOutputDirectoryArg("termsurf")
        .path(b, b.fmt("termsurf-{f}", .{cfg.version}));
...
/// of the source tarball with TermSurf. These aren't committed to the Git
