% TERMSURF(5) Version @@VERSION@@ | TermSurf terminal emulator configuration file

# NAME

**termsurf** - TermSurf terminal emulator configuration file

# DESCRIPTION

To configure TermSurf, you must use a configuration file. GUI-based configuration
is on the roadmap but not yet supported. The configuration file must be placed
at `$XDG_CONFIG_HOME/termsurf/config.ghostty`, which defaults to `~/.config/termsurf/config.ghostty`
...
`$HOME/Library/Application Support/com.termsurf/config.ghostty`.** This is the
...
executing the command `termsurf +show-config --default --docs`. Note that this will
...
the public config files of many TermSurf users for examples and inspiration.

## Configuration Errors

If your configuration file has any errors, TermSurf does its best to ignore
...
macOS and Linux (GTK). TermSurf does not treat configuration errors as fatal and
will fall back to default values for erroneous keys.

You can also view the full configuration TermSurf is loading using `termsurf
...
--Use the system `log` CLI to view TermSurf's logs: `sudo log stream --level debug
--predicate 'subsystem=="com.termsurf"'`.
