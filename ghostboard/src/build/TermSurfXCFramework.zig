const TermSurfXCFramework = @This();

const std = @import("std");
const Config = @import("Config.zig");
const SharedDeps = @import("SharedDeps.zig");
const TermSurfLib = @import("TermSurfLib.zig");
...
pub fn init(
    ...
    const macos_universal = try TermSurfLib.initMacOSUniversal(b, deps);

    // Native macOS build
    const macos_native = try TermSurfLib.initStatic(b, &try deps.retarget(
        b,
        Config.genericMacOSTarget(b, null),
    ));

    // iOS
    const ios = try TermSurfLib.initStatic(b, &try deps.retarget(
        b,
    ...
    // iOS Simulator
    const ios_sim = try TermSurfLib.initStatic(b, &try deps.retarget(
        b,
        b.resolveTargetQuery(.{
            .cpu_arch = .aarch64,
            .os_tag = .ios,
    ...
    // The xcframework wraps our termsurf library so that we can link
    // it to the final app built with Swift.
    const xcframework = XCFrameworkStep.create(b, .{
        .name = "TermSurfKit",
        .out_path = "macos/TermSurfKit.xcframework",
