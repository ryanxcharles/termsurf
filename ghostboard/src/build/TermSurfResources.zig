...
pub fn init(b: *std.Build, cfg: *const Config, deps: *const SharedDeps) !TermSurfResources {
    ...
    terminfo: {
        ...
        const source = wf.addCopyFile(run.captureStdOut(), "termsurf.terminfo");

        if (cfg.emit_terminfo) {
            const source_install = b.addInstallFile(
                source,
                if (os_tag == .freebsd)
                    "share/site-terminfo/termsurf.terminfo"
                else
                    "share/terminfo/termsurf.terminfo",
            );
        ...
        if (cfg.emit_termcap) {
            ...
            const cap_install = b.addInstallFile(
                out_source,
                if (os_tag == .freebsd)
                    "share/site-terminfo/termsurf.termcap"
                else
                    "share/terminfo/termsurf.termcap",
            );
    ...
    // Shell-integration
    {
        const install_step = b.addInstallDirectory(.{
            ...
            .install_subdir = b.pathJoin(&.{ "termsurf", "shell-integration" }),
    ...
    if (cfg.emit_themes) {
        if (b.lazyDependency("iterm2_themes", .{})) |upstream| {
            const install_step = b.addInstallDirectory(.{
                ...
                .install_dir = .{ .custom = "share" },
                .install_subdir = b.pathJoin(&.{ "termsurf", "themes" }),
                .exclude_extensions = &.{".md"},
            });
            try steps.append(b.allocator, &install_step.step);
        }
    ...
    // Fish shell completions
    {
        ...
        run.addArg("+fish");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(), "termsurf.fish");
    ...
    // zsh shell completions
    {
        ...
        run.addArg("+zsh");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(), "_termsurf");
    ...
    // bash shell completions
    {
        ...
        run.addArg("+bash");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(), "termsurf.bash");
    ...
    // Vim and Neovim plugin
    {
...

        {
            ...
            _ = wf.addCopyFile(run.captureStdOut(), "syntax/termsurf.vim");
        }
        {
            const run = b.addRunArtifact(build_data_exe);
            run.addArg("+vim-ftdetect");
            _ = wf.addCopyFile(run.captureStdOut(), "ftdetect/termsurf.vim");
        }
        {
            const run = b.addRunArtifact(build_data_exe);
            run.addArg("+vim-ftplugin");
            _ = wf.addCopyFile(run.captureStdOut(), "ftplugin/termsurf.vim");
        }
        {
            const run = b.addRunArtifact(build_data_exe);
            run.addArg("+vim-compiler");
            _ = wf.addCopyFile(run.captureStdOut(), "compiler/termsurf.vim");
        }
    ...
    // 'termsurf.sublime-syntax' file from zig-out to the '~.config/bat/syntaxes'
    // directory. The syntax then needs to be mapped to the correct language in
    // the config file within the '~.config/bat' directory
    // (ex: --map-syntax "/Users/user/.config/termsurf/config.ghostty:TermSurf Config").
    {
        const run = b.addRunArtifact(build_data_exe);
        run.addArg("+sublime");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(), "termsurf.sublime-syntax");
...
/// Add the resource files needed to make TermSurf a proper
...
fn addLinuxAppResources(
    ...
    const app_id = b.fmt("com.termsurf{s}", .{
        switch (cfg.optimize) {
            .Debug, .ReleaseSafe => "-debug",
            .ReleaseFast, .ReleaseSmall => "",
        },
    ...
    const templates: []const Template = templates: {
        ...
        // within app stores
        try ts.append(b.allocator, .{
            b.path("dist/linux/com.termsurf.metainfo.xml.in"),
    ...
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("dist/linux/termsurf_dolphin.desktop"),
        "share/kio/servicemenus/com.termsurf.desktop",
    ).step);

    // Right click menu action for Nautilus. Note that this _must_ be named
    // `termsurf.py`. Using the full app id causes problems (see #5468).
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("dist/linux/termsurf_nautilus.py"),
        "share/nautilus-python/extensions/termsurf.py",
    ).step);
...
pub fn install(self: *const TermSurfResources) void {
    const b = self.steps[0].owner;
    self.addStepDependencies(b.getInstallStep());
}

pub fn addStepDependencies(
    self: *const TermSurfResources,
