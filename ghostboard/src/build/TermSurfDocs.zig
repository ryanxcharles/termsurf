//! TermSurfDocs generates all the on-disk documentation that TermSurf is
//! installed with (man pages, html, markdown, etc.)
const TermSurfDocs = @This();
...
pub fn init(
    ...
    const manpages = [_]struct {
        ...
        .{ .name = "termsurf", .section = "1" },
        .{ .name = "termsurf", .section = "5" },
    };
    ...
    inline for (manpages) |manpage| {
        ...
        try steps.append(b.allocator, &b.addInstallFile(
            markdown_output,
            "share/termsurf/doc/" ++ manpage.name ++ "." ++ manpage.section ++ ".md",
        ).step);
        ...
        try steps.append(b.allocator, &b.addInstallFile(
            ...
            "share/termsurf/doc/" ++ manpage.name ++ "." ++ manpage.section ++ ".html",
        ).step);
...
/// app.
pub fn installDummy(self: *const TermSurfDocs, step: *std.Build.Step) void {
    _ = self;
