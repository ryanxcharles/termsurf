...
pub fn init(b: *std.Build, cfg: *const Config) !TermSurfI18n {
    _ = cfg;
...
pub fn install(self: *const TermSurfI18n) void {
    self.addStepDependencies(self.owner.getInstallStep());
}

pub fn addStepDependencies(
    self: *const TermSurfI18n,
...
fn createUpdateStep(b: *std.Build) !*std.Build.Step {
    ...
    inline for (gresource.blueprints) |blp| {
        ...
        // a lot of churn because not everyone has the TermSurf code checked out in
...

    {
        ...
        for (gtk_files.items) |item| {
            ...
            // cause a lot of churn because not everyone has the TermSurf code
    ...
    const nautilus_script_path = "dist/linux/termsurf_nautilus.py";
