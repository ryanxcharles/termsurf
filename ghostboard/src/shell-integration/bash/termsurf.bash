...
if [ -n "$TERMSURF_BASH_INJECT" ]; then
  ...
  builtin declare __termsurf_bash_flags="$TERMSURF_BASH_INJECT"
  builtin unset ENV TERMSURF_BASH_INJECT

  # Restore an existing ENV that was replaced by the shell integration code.
  if [[ -n "$TERMSURF_BASH_ENV" ]]; then
    builtin export ENV=$TERMSURF_BASH_ENV
    builtin unset TERMSURF_BASH_ENV
  fi
  ...
  if [[ -n "$TERMSURF_BASH_UNEXPORT_HISTFILE" ]]; then
    builtin export -n HISTFILE
    builtin unset TERMSURF_BASH_UNEXPORT_HISTFILE
  fi
  ...
  if builtin shopt -q login_shell; then
    if [[ $__termsurf_bash_flags != *"--noprofile"* ]]; then
      [ -r /etc/profile ] && builtin source "/etc/profile"
      for __termsurf_rcfile in "$HOME/.bash_profile" "$HOME/.bash_login" "$HOME/.profile"; do
        [ -r "$__termsurf_rcfile" ] && {
          builtin source "$__termsurf_rcfile"
          break
        }
      done
    fi
  else
    if [[ $__termsurf_bash_flags != *"--norc"* ]]; then
      ...
      for __termsurf_rcfile in /etc/bash.bashrc /etc/bash/bashrc /etc/bashrc; do
        [ -r "$__termsurf_rcfile" ] && {
          builtin source "$__termsurf_rcfile"
          break
        }
      done
      if [[ -z "$TERMSURF_BASH_RCFILE" ]]; then TERMSURF_BASH_RCFILE="$HOME/.bashrc"; fi
      [ -r "$TERMSURF_BASH_RCFILE" ] && builtin source "$TERMSURF_BASH_RCFILE"
    fi
  fi

  builtin unset __termsurf_rcfile
  builtin unset __termsurf_bash_flags
  builtin unset TERMSURF_BASH_RCFILE
fi

# Add TermSurf binary to PATH if the path feature is enabled
if [[ "$TERMSURF_SHELL_FEATURES" == *"path"* && -n "$TERMSURF_BIN_DIR" ]]; then
  if [[ ":$PATH:" != *":$TERMSURF_BIN_DIR:"* ]]; then
    export PATH="$PATH:$TERMSURF_BIN_DIR"
  fi
fi

# Sudo
if [[ "$TERMSURF_SHELL_FEATURES" == *"sudo"* && -n "$TERMINFO" ]]; then
  # Wrap `sudo` command to ensure TermSurf terminfo is preserved.
  #
...
# SSH Integration
if [[ "$TERMSURF_SHELL_FEATURES" == *ssh-* ]]; then
  function ssh() {
    ...
    if [[ "$TERMSURF_SHELL_FEATURES" == *ssh-terminfo* ]]; then
      ...
      if [[ -n "$ssh_hostname" ]]; then
        ...
        if "$TERMSURF_BIN_DIR/termsurf" +ssh-cache --host="$ssh_target" >/dev/null 2>&1; then
          ssh_term="xterm-termsurf"
        elif builtin command -v infocmp >/dev/null 2>&1; then
          builtin local ssh_terminfo ssh_cpath_dir ssh_cpath

          ssh_terminfo=$(infocmp -0 -x xterm-termsurf 2>/dev/null)

          if [[ -n "$ssh_terminfo" ]]; then
            builtin echo "Setting up xterm-termsurf terminfo on $ssh_hostname..." >&2

            ssh_cpath_dir=$(mktemp -d "/tmp/termsurf-ssh-$ssh_user.XXXXXX" 2>/dev/null) || ssh_cpath_dir="/tmp/termsurf-ssh-$ssh_user.$$"
            ...
            ' 2>/dev/null; then
              ...
              "$TERMSURF_BIN_DIR/termsurf" +ssh-cache --add="$ssh_target" >/dev/null 2>&1 || true
            else
          ...
          builtin echo "Warning: termsurf command not available for cache management." >&2
        fi
      fi
    fi
...
_termsurf_executing=""
_termsurf_last_reported_cwd=""

function __termsurf_precmd() {
  local ret="$?"
  if test "$_termsurf_executing" != "0"; then
    _TERMSURF_SAVE_PS1="$PS1"
    _TERMSURF_SAVE_PS2="$PS2"
    ...
    if [[ "${PS1}" == *"\n"* || "${PS1}" == *$'\n'* ]]; then
      builtin local __termsurf_mark=$'\\[\\e]133;A;k=s\\a\\]'
      PS1="${PS1//$'\n'/$'\n'$__termsurf_mark}"
      PS1="${PS1//\\n/\\n$__termsurf_mark}"
    fi

    # Cursor
    if [[ "$TERMSURF_SHELL_FEATURES" == *"cursor"* ]]; then
      builtin local cursor=5  # blinking bar
      [[ "$TERMSURF_SHELL_FEATURES" == *"cursor:steady"* ]] && cursor=6  # steady bar
    ...
    if [[ "$TERMSURF_SHELL_FEATURES" == *"title"* ]]; then
      PS1=$PS1'\[\e]2;\w\a\]'
    fi
  fi

  if test "$_termsurf_executing" != ""; then
  ...
  if [[ "$_termsurf_last_reported_cwd" != "$PWD" ]]; then
    _termsurf_last_reported_cwd="$PWD"
    builtin printf "\e]7;kitty-shell-cwd://%s%s\a" "$HOSTNAME" "$PWD"
  fi

  _termsurf_executing=0
}

function __termsurf_preexec() {
  builtin local cmd="$1"

  PS1="$_TERMSURF_SAVE_PS1"
  PS2="$_TERMSURF_SAVE_PS2"

  # Title (current command)
  if [[ -n $cmd && "$TERMSURF_SHELL_FEATURES" == *"title"* ]]; then
    builtin printf "\e]2;%s\a" "${cmd//[[:cntrl:]]/}"
  fi

  # End of input, start of output.
  builtin printf "\e]133;C;\a"
  _termsurf_executing=1
}

if (( BASH_VERSINFO[0] > 4 || (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] >= 4) )); then
  __termsurf_preexec_hook() {
    builtin local cmd
    cmd=$(LC_ALL=C HISTTIMEFORMAT='' builtin history 1)
    cmd="${cmd#*[[:digit:]][* ] }"  # remove leading history number
    [[ -n "$cmd" ]] && __termsurf_preexec "$cmd"
  }
  ...
  if (( BASH_VERSINFO[0] > 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] >= 3) )); then
    ...
    builtin readonly __termsurf_ps0='${ __termsurf_preexec_hook; }'
  else
    # shellcheck disable=SC2016
    builtin readonly __termsurf_ps0='$(__termsurf_preexec_hook >/dev/tty)'
  fi

  __termsurf_hook() {
    builtin local ret=$?
    __termsurf_precmd "$ret"
    if [[ "$PS0" != *"$__termsurf_ps0"* ]]; then
      PS0=$PS0"${__termsurf_ps0}"
    fi
  }

  # Append our hook to PROMPT_COMMAND, preserving its existing type.
  if [[ ";${PROMPT_COMMAND[*]:-};" != *";__termsurf_hook;"* ]]; then
    if [[ -z "${PROMPT_COMMAND[*]}" ]]; then
      if (( BASH_VERSINFO[0] > 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] >= 1) )); then
        PROMPT_COMMAND=(__termsurf_hook)
      else
  ...
  preexec_functions+=(__termsurf_preexec)
  precmd_functions+=(__termsurf_precmd)
fi
