{
  ...
  if (and (has-env TERMSURF_SHELL_INTEGRATION_XDG_DIR) (has-env XDG_DATA_DIRS)) {
    set-env XDG_DATA_DIRS (str:replace $E:TERMSURF_SHELL_INTEGRATION_XDG_DIR":" "" $E:XDG_DATA_DIRS)
    unset-env TERMSURF_SHELL_INTEGRATION_XDG_DIR
  }

  # List of enabled shell integration features
  var features = [(str:split ',' $E:TERMSURF_SHELL_FEATURES)]

  # helper used by `mark-*` functions
  fn set-prompt-state {|new| set-env __termsurf_prompt_state $new }

  fn mark-prompt-start {
    if (not-eq prompt-start (constantly $E:__termsurf_prompt_state)) {
      printf "\e]133;D\a"
    }
  ...
  fn ssh-integration {|@args|
    ...
    if (has-value $features ssh-terminfo) {
      ...
      if (not-eq $ssh-hostname "") {
        var termsurf = $E:TERMSURF_BIN_DIR/"termsurf"
        var ssh-target = $ssh-user"@"$ssh-hostname

        # Check if terminfo is already cached
        if (bool ?($termsurf +ssh-cache --host=$ssh-target)) {
          set ssh-term = "xterm-termsurf"
        } elif (has-external infocmp) {
          var ssh-terminfo = ((external infocmp) -0 -x xterm-termsurf 2>/dev/null | slurp)

          if (not-eq $ssh-terminfo "") {
            echo "Setting up xterm-termsurf terminfo on "$ssh-hostname"..." >&2

            use os
            var ssh-cpath-dir = (os:temp-dir "termsurf-ssh-"$ssh-user".*")
            ...
            if (bool ?(echo $ssh-terminfo | (external ssh) $@ssh-opts -o ControlMaster=yes -o ControlPath=$ssh-cpath -o ControlPersist=60s $@args '
              ...
              $termsurf +ssh-cache --add=$ssh-target >/dev/null
            } else {
  ...
  if (str:contains $E:TERMSURF_SHELL_FEATURES "cursor") {
  ...
  if (and (has-value $features path) (has-env TERMSURF_BIN_DIR)) {
    if (not (has-value $paths $E:TERMSURF_BIN_DIR)) {
        set paths = [$@paths $E:TERMSURF_BIN_DIR]
    }
  }
  if (and (has-value $features sudo) (not-eq "" $E:TERMINFO) (has-external sudo)) {
    edit:add-var sudo~ $sudo-with-terminfo~
  }
  if (and (str:contains $E:TERMSURF_SHELL_FEATURES ssh-) (has-external ssh)) {
