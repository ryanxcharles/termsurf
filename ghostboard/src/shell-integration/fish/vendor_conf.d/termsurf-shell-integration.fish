...
function termsurf_restore_xdg_data_dir -d "restore the original XDG_DATA_DIR value"
    # If we don't have our own data dir then we don't need to do anything.
    if not set -q TERMSURF_SHELL_INTEGRATION_XDG_DIR
        return
    end
    ...
    if set --function index (contains --index "$TERMSURF_SHELL_INTEGRATION_XDG_DIR" $xdg_data_dirs)
    ...
    set --erase TERMSURF_SHELL_INTEGRATION_XDG_DIR
end

function termsurf_exit -d "exit the shell integration setup"
    functions -e termsurf_restore_xdg_data_dir
    functions -e termsurf_exit
    exit 0
end

# We always try to restore the XDG data dir
termsurf_restore_xdg_data_dir

# If we aren't interactive or we've already run, don't run.
status --is-interactive || termsurf_exit
...
function __termsurf_setup --on-event fish_prompt -d "Setup termsurf integration"
    functions -e __termsurf_setup

    set --local features (string split , $TERMSURF_SHELL_FEATURES)
    ...
    set -g __termsurf_prompt_start_mark "\e]133;A\a"
    if test "$fish_major" -gt 4; or test "$fish_major" -eq 4 -a "$fish_minor" -ge 1
        set -g __termsurf_prompt_start_mark "\e]133;A;click_events=1\a"
    end
    ...
    if string match -q 'cursor*' -- $features
        ...
        function __termsurf_set_cursor_beam --on-event fish_prompt -V cursor -d "Set cursor shape"
            if not functions -q fish_vi_cursor_handle
                echo -en "\e[$cursor q"
            end
        end
        function __termsurf_reset_cursor --on-event fish_preexec -d "Reset cursor shape"
            if not functions -q fish_vi_cursor_handle
                echo -en "\e[0 q"
            end
        end
    ...
    if contains path $features; and test -n "$TERMSURF_BIN_DIR"
        fish_add_path --global --path --append "$TERMSURF_BIN_DIR"
    end
    ...
    # SSH Integration
    set -l features (string split ',' -- "$TERMSURF_SHELL_FEATURES")
    if contains ssh-env $features; or contains ssh-terminfo $features
        function ssh --wraps=ssh --description "SSH wrapper with TermSurf integration"
            set -l features (string split ',' -- "$TERMSURF_SHELL_FEATURES")
            ...
            if contains ssh-terminfo $features
                ...
                if test -n "$ssh_hostname"
                    ...
                    if test -x "$TERMSURF_BIN_DIR/termsurf"; and "$TERMSURF_BIN_DIR/termsurf" +ssh-cache --host="$ssh_target" >/dev/null 2>&1
                        set ssh_term xterm-termsurf
                    else if command -q infocmp
                        set -l ssh_terminfo
                        set -l ssh_cpath_dir
                        set -l ssh_cpath

                        set ssh_terminfo "$(infocmp -0 -x xterm-termsurf 2>/dev/null)"

                        if test -n "$ssh_terminfo"
                            echo "Setting up xterm-termsurf terminfo on $ssh_hostname..." >&2

                            set ssh_cpath_dir (mktemp -d "/tmp/termsurf-ssh-$ssh_user.XXXXXX" 2>/dev/null; or echo "/tmp/termsurf-ssh-$ssh_user."(random))
                            ...
                            ' 2>/dev/null
                                ...
                                if test -x "$TERMSURF_BIN_DIR/termsurf"
                                    "$TERMSURF_BIN_DIR/termsurf" +ssh-cache --add="$ssh_target" >/dev/null 2>&1; or true
                                end
                            else
                        ...
                        echo "Warning: termsurf command not available for cache management." >&2
                    end
                end
            end
    ...
    function __termsurf_mark_prompt_start --on-event fish_prompt --on-event fish_posterror
        # If we never got the output end event, then we need to send it now.
        if test "$__termsurf_prompt_state" != prompt-start
            echo -en "\e]133;D\a"
        end

        set --global __termsurf_prompt_state prompt-start
        echo -en $__termsurf_prompt_start_mark
    end

    function __termsurf_mark_output_start --on-event fish_preexec
        set --global __termsurf_prompt_state pre-exec
        echo -en "\e]133;C\a"
    end

    function __termsurf_mark_output_end --on-event fish_postexec
        set --global __termsurf_prompt_state post-exec
    ...
    # Enable fish to handle reflow because TermSurf clears the prompt on resize.
    set --global fish_handle_reflow 1

    # Initial calls for first prompt
    if string match -q 'cursor*' -- $features
        __termsurf_set_cursor_beam
    end
    __termsurf_mark_prompt_start
    __update_cwd_osc
end
