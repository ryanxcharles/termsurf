...
def open_in_termsurf_activated(_menu, paths):
    for path in paths:
        cmd = ['termsurf', f'--working-directory={path}', '--gtk-single-instance=false']
...
def get_items_for_files(name, files):
    ...
    if paths:
        item = Nautilus.MenuItem(name=name, label=_('Open in TermSurf'),
            icon='com.termsurf')
        item.connect('activate', open_in_termsurf_activated, paths)
        return [item]
    else:
        return []


class TermSurfMenuProvider(GObject.GObject, Nautilus.MenuProvider):
    def get_file_items(self, files):
        return get_items_for_files('TermSurfNautilus::open_in_termsurf', files)

    def get_background_items(self, file):
        return get_items_for_files('TermSurfNautilus::open_folder_in_termsurf', [file])
