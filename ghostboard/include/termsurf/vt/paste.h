...
#ifndef TERMSURF_VT_PASTE_H
#define TERMSURF_VT_PASTE_H
 ...
 * ## Basic Usage
 *
 * Use termsurf_paste_is_safe() to check if paste data contains potentially
 ...
 * 
 *   if (termsurf_paste_is_safe(safe_data, strlen(safe_data))) {
 *     printf("Safe to paste\n");
 *   }
 * 
 *   if (!termsurf_paste_is_safe(unsafe_data, strlen(unsafe_data))) {
 ...
 */
bool termsurf_paste_is_safe(const char* data, size_t len);
