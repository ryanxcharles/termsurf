...
pub fn connect(path: &str) -> Option<UnixStream> {
    let stream = match UnixStream::connect(path) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("[Surfari] connect failed: {e}");
            return None;
        }
    };
...
pub fn listen(path: &str) {
    ...
    let listener = match UnixListener::bind(path) {
        Ok(l) => l,
        Err(e) => {
            eprintln!("[Surfari] listener bind failed: {e}");
            return;
        }
    };
    eprintln!("[Surfari] listener bound: {path}");

    std::thread::spawn(move || {
        for conn in listener.incoming() {
            match conn {
                Ok(stream) => {
                    eprintln!("[Surfari] client connected");
                    let reader = match stream.try_clone() {
                        Ok(r) => r,
                        Err(e) => {
                            eprintln!("[Surfari] clone failed: {e}");
                            continue;
                        }
                    };
                ...
                Err(e) => {
                    eprintln!("[Surfari] accept error: {e}");
                }
            }
        }
    });
...
fn read_messages(mut stream: UnixStream, quit_on_eof: bool) {
    ...
    loop {
        let n = match stream.read(&mut tmp) {
            Ok(0) => {
                if quit_on_eof {
                    eprintln!("[Surfari] socket EOF — requesting quit");
                    unsafe { ffi::ts_post_task(Some(quit_trampoline), std::ptr::null_mut()) };
                } else {
                    eprintln!("[Surfari] client disconnected");
                }
                return;
            }
            Ok(n) => n,
            Err(e) => {
                if quit_on_eof {
                    eprintln!("[Surfari] socket read error: {e} — requesting quit");
                    unsafe { ffi::ts_post_task(Some(quit_trampoline), std::ptr::null_mut()) };
                } else {
                    eprintln!("[Surfari] client read error: {e}");
                }
                return;
            }
        };
