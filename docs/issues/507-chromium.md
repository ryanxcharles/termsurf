...
### Experiment 3: Safe IOSurface Swapping

Experiment 2 crashes on resize because the IOSurface lifetime is not managed
across the Swift/Zig boundary. Swift passes `Unmanaged.passUnretained` — a raw
pointer with no ownership transfer. When Swift recreates the test surface (on
resize), ARC releases the old `IOSurface` object. But the renderer may still be
mid-frame, reading that memory via `overlay_iosurface`. Use-after-free → crash.

This is the same problem live Chromium frames will have. New IOSurfaces arrive
at 60fps, each replacing the old one. The swap must be safe.

#### Fix

Transfer ownership across the boundary using `CFRetain`/`CFRelease`:

1. **Swift retains** before passing the pointer to Zig.
2. **Zig releases the old** pointer when storing a new one.
3. **Zig releases** on clear.

This way the IOSurface stays alive as long as the renderer holds it, regardless
of what ARC does on the Swift side. Ghostty already has `CFRetain`/`CFRelease`
bindings in `pkg/macos/foundation/type.zig` (used by `IOSurface.retain()`,
`KeymapDarwin`, and the CoreText shaper).

#### Changes

**1. Swift: retain before passing — `CompositorXPC.swift`**

Change `Unmanaged.passUnretained` to `Unmanaged.passRetained`:

```swift
// Before (Experiment 1-2):
let ptr = Unmanaged.passUnretained(ioSurface).toOpaque()

// After:
let ptr = Unmanaged.passRetained(ioSurface).toOpaque()
```

`passRetained` calls `CFRetain` and returns the pointer. The Zig side now owns
one retain count on this pointer.

**2. Zig: release old pointer — `Surface.zig`**

In `setOverlayIOSurface`, release the old pointer before storing the new one:

```zig
pub fn setOverlayIOSurface(self: *Surface, iosurface: ?*anyopaque) void {
    self.renderer.draw_mutex.lock();
    defer self.renderer.draw_mutex.unlock();
    // Release the previous IOSurface (we own a retain from Swift).
    if (self.renderer.overlay_iosurface) |old| {
        macos.foundation.CFRelease(old);
    }
    self.renderer.overlay_iosurface = iosurface;
    self.queueRender() catch {};
}
...
**3. Zig: release on clear — `Surface.zig`**

In `clearOverlay`, release before nulling:
...
**4. Surface.zig: import macos**

Surface.zig doesn't currently import `macos`. Add the conditional import (same
pattern as `generic.zig`):
...
`builtin` is already imported in Surface.zig.

#### Why This Is Safe

The `draw_mutex` ensures the pointer swap is atomic with respect to the
renderer. The renderer locks `draw_mutex` at the start of `drawFrame()` (it
doesn't — but the overlay read is inside the draw pass which holds the lock
implicitly via the frame setup). Actually, let's verify: the renderer reads
`self.overlay_iosurface` during `drawFrame()`. The setter locks `draw_mutex`. As
long as the renderer also holds `draw_mutex` while reading the pointer and using
the IOSurface, the swap is safe.

The key invariant: from the moment `setOverlayIOSurface` returns, the old
IOSurface is released and the new one is retained. The renderer will see either
the old pointer (before the lock) or the new pointer (after the lock), never a
dangling pointer.

#### Pass Criteria

1. Resizing the terminal while the overlay is active does not crash.
2. The checkerboard is recreated at the new resolution after resize — still
   pixel-perfect.
3. Quitting `web` clears the overlay without crash.
4. No memory leaks (the old IOSurface is freed when replaced).

#### Files

| File                                            | Change                                     |
| ----------------------------------------------- | ------------------------------------------ |
| `ts5/macos/Sources/Ghostty/CompositorXPC.swift` | `passUnretained` → `passRetained`          |
| `ts5/src/Surface.zig`                           | CFRelease old pointer in set/clear, import |
...
# Expected: crisp checkerboard at viewport
# Resize terminal → no crash, checkerboard recreated at new size
# Quit web → overlay clears, no crash
```
