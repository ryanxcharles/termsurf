...
#### Phase 1: Enable Multi-Threaded Message Loop

**Changes required:**

1. **Enable the setting** in `termsurf-profile/src/main.rs`:
...
2. **Remove manual polling loop** — replace:
...
With either `cef::run_message_loop()` or a simple wait on the quit flag.

3. **Verify thread safety** — ensure XPC handlers and `on_accelerated_paint`
   callbacks handle being called from CEF's internal thread.

**Risks:**

- Thread safety issues if XPC handlers assume single-threaded execution
- `on_accelerated_paint` may be called from a different thread
- May require additional synchronization for shared state
...
If Option A fails, proceed to Option C: enable `external_begin_frame_enabled`
and call `browser_host.send_external_begin_frame()` at 60Hz ourselves.
