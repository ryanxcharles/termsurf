...
Add timing logs to measure current performance before making changes.

**Instrumentation to add:**

1. **IOSurface import time** — In `draw.rs`, wrap the import calls:
   ```rust
   let import_start = std::time::Instant::now();
   let importer = IOSurfaceImporter::from_mach_port(...)?;
   let texture = importer.import_to_wgpu(&webgpu.device)?;
   log::info!("[PERF] IOSurface import: {:?}", import_start.elapsed());
   ```

2. **Total webview render time** — Wrap `render_webview_overlays_webgpu()`:
   ```rust
   let webview_start = std::time::Instant::now();
   self.render_webview_overlays_webgpu(webgpu, &output.texture)?;
   log::info!("[PERF] Webview render total: {:?}", webview_start.elapsed());
   ```

3. **Mach port changes** — Log when port actually changes vs reused:
   ```rust
   log::info!("[PERF] Mach port: {} (changed={})", surface.mach_port, changed);
   ```

**Baseline data to collect:**

- [ ] Average IOSurface import time (expect 10-50ms)
- [ ] How often Mach port changes during scrolling
- [ ] Total webview render time per frame

#### Phase 2: Implement Caching

After collecting baseline data:

1. Add `cached_textures: HashMap<u32, CachedTexture>` to render state
   ```rust
...
3. If match AND dimensions match, reuse existing texture
4. If not, import and cache new texture
5. Log cache hits vs misses for measurement

#### Phase 3: Measure Improvement

Compare instrumented metrics before/after:

- [ ] IOSurface import time (should drop to ~0ms on cache hit)
- [ ] Cache hit rate during scrolling (expect high)
- [ ] Total webview render time (should decrease significantly)

**Success criteria:**

- [ ] Cache hit rate > 90% during normal scrolling
- [ ] Frame render time reduced by 10-50ms on cache hits
