...
Create a new agent skill for TermSurf. Every new skill must follow the Agent
...
2. **Read every existing skill.** Read all `SKILL.md` files in
   `.claude/skills/*/SKILL.md` to understand the project's conventions, tone,
   and level of detail.

3. **Create the skill directory and file.** The skill is a directory in
   `.claude/skills/<name>/` containing a `SKILL.md` file. The directory name
...
5. **Match existing conventions.** Based on the current skills:
   ...
   - Reference specific paths in the TermSurf repo where relevant
