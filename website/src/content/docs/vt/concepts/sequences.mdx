...
| `ESC X`  | `0x98` | SOS  | Start of String             | Obsolete. Ignored by TermSurf.                                                                           |
| `ESC ^`  | `0x9e` | PM   | Private Message             | Obsolete. Ignored by TermSurf.                                                                           |
...
non-numeric identifiers before the semicolon, but TermSurf does not support these.

> [!NOTE]
> Due to yet more convoluted historical reasons, OSC sequences in xterm (and
> by extension TermSurf) may be terminated with the Bell character (`BEL`,
> `0x07`) instead of the standard String Terminator (`ST`, `ESC \` or `0x9c`).
>
> While they are in most cases interchangeable, TermSurf will try to echo back
...
An example OSC sequence is `OSC 2 ; 👻  TermSurf 👻  ST` (where `ST` is
...
TermSurf supports APC sequences, most notably the Kitty Graphics Protocol
sequence. This is documented [here](https://sw.kovidgoyal.net/kitty/graphics-protocol/).
...
SOS and PM sequences are entirely ignored by TermSurf.
