...
TermSurf attempts to differentiate between the two by trying to find the
...
questionable security-wise at worst, TermSurf only *implements* the extensions
that are listed below. There are many more that TermSurf does parse, but either
due to security reasons or the fact that TermSurf does not try to be a ConEmu
...
error, or indeterminate, which is represented visually in TermSurf as a progress
...
> cleaning up its progress state. This is why TermSurf has a **hardcoded
