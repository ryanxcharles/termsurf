...
# Experiment 602: search reload_active pin accessors (pin_before, active_top_left)
...
focused `PageList::active_top_left` wrapper over the private `get_top_left`, so
its visibility isn't widened). It extends `terminal::page_list` and
`terminal::screen`.
...
impl PageList {
    ...
    pub(in crate::terminal) fn active_top_left(&self) -> Pin {
...
- **Ported (exposed)**: `Pin.before` → reuse the existing `Screen::pin_before`
  ...
  `Screen::active_top_left` (over a new focused `PageList::active_top_left`
  wrapping the private `get_top_left`).
- **Faithful**: `pin_before`'s same-node `y`/`x` ordering and different-node
  ...
  matches upstream's forward node walk); `active_top_left` returning the active
  area's top-left pin.
- **Faithful adaptation**: `pin_before` returns `Option<bool>` (`None` for an
  ...
  pruning skips). `get_top_left` stays private; `active_top_left` is the focused
  public entry point reload_active needs (only the `.active` tag).
...
1. `roastty/src/terminal/page_list.rs`: add `PageList::active_top_left`
   (wrapping the private `get_top_left(Active)`).
2. `roastty/src/terminal/screen.rs`: add `Screen::active_top_left`.
...
3. Tests:
   ...
   - **`active_top_left`** (in `screen.rs`): for `Screen::init(10, 10, None)`,
     `active_top_left()` is the first node at `(x: 0, y: 0)`, and it is
     `pin_before` a lower row (`(node, 5, 0)`) → `Some(true)`.
...
- `Screen::pin_before` reflects screen-order comparison (same-node `y`/`x`,
  ...
  `Screen::active_top_left` returns the active top-left pin — faithful to the
  `Pin.before` / `getTopLeft(.active)` usage in `terminal/search/screen.zig`;
- the tests pass (`pin_before` ordering / `active_top_left`), and the existing
  tests still pass;
...
local invalid/garbage-pin adaptation; and a focused `PageList::active_top_left`
wrapper around the private `get_top_left(Tag::Active)` is the right boundary
(reload only needs the active top-left, not generic tag access). One Required
and one Optional, both adopted:
