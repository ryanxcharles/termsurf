//! Test Sender for Experiment 1.
//!
//! Spawned by launcher, claims session to get receiver endpoint, creates
//! IOSurface and sends Mach port to receiver.
//!
//! Build with: cargo build --release --example sender
...
fn main() {
    ...
    let receiver_endpoint = match claim_session_with_retry(&launcher, &args.session) {
        Ok(ep) => {
    ...
    let receiver = match XpcConnection::from_endpoint(receiver_endpoint) {
        Ok(c) => {
            println!("Sender: Connected to receiver");
            c
        }
        Err(e) => {
            eprintln!("Sender: Failed to connect to receiver: {}", e);
            std::process::exit(1);
        }
    };
    ...
    // 5. Create Mach port and send to receiver
    ...
    receiver.send(&msg);
    println!("Sender: Sent IOSurface Mach port to receiver");
