...
LOG_DIR="$ROOT/logs/issue-834-exp53-pdf-view-geometry-compare"
RUN_DIR="$(mktemp -d "${TMPDIR:-/tmp}/termsurf-issue834-exp53.XXXXXX")"
SITE_DIR="$RUN_DIR/site"
PDF_PATH="$SITE_DIR/separated-tokens.pdf"
SWIFT_APP="$RUN_DIR/GeometryProbe.swift"
PROBE_APP="$RUN_DIR/geometry-probe"
TRIGGER="$RUN_DIR/geometry.trigger"
ACK="$RUN_DIR/geometry.ack"
STANDALONE_TRACE="$LOG_DIR/standalone-wkpdf-geometry-$RUN_ID.log"
STANDALONE_STDOUT="$LOG_DIR/standalone-wkpdf-stdout-$RUN_ID.log"
STANDALONE_STDERR="$LOG_DIR/standalone-wkpdf-stderr-$RUN_ID.log"
EMBEDDED_TRACE="$LOG_DIR/embedded-surfari-geometry-$RUN_ID.log"
EMBEDDED_COPY_TRACE="$LOG_DIR/embedded-surfari-copy-$RUN_ID.log"
SUMMARY="$LOG_DIR/pdf-view-geometry-compare-summary.json"
...
func rect(_ value: NSRect) -> String {
    NSStringFromRect(value)
}

func point(_ value: NSPoint) -> String {
...
func viewTree(_ view: NSView?, _ depth: Int = 0) -> String {
    ...
    var items = [
        "\(type(of: view)):\(Unmanaged.passUnretained(view).toOpaque()) frame=\(rect(view.frame)) bounds=\(rect(view.bounds)) hidden=\(view.isHidden) alpha=\(view.alphaValue) layered=\(view.wantsLayer)"
    ]
...
func scrollViews(_ view: NSView?) -> String {
    ...
    if let scroll = view as? NSScrollView {
        items.append("\(type(of: scroll)):\(Unmanaged.passUnretained(scroll).toOpaque()) frame=\(rect(scroll.frame)) bounds=\(rect(scroll.bounds)) document=\(describe(scroll.documentView)) document_frame=\(rect(scroll.documentView?.frame ?? .zero)) document_bounds=\(rect(scroll.documentView?.bounds ?? .zero)) clip_bounds=\(rect(scroll.contentView.bounds))")
    }
    for subview in view.subviews {
...
final class Delegate: NSObject, NSApplicationDelegate, WKNavigationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        window.title = "Issue 834 Exp53 WKWebView"
    ...
    func dump(_ label: String, webX: CGFloat, webY: CGFloat) {
        ...
        appendTrace("standalone-pdf-view-geometry-state label=\(label) url=\(webView.url?.absoluteString ?? "") input=\(webX),\(webY) window_point=\(point(windowPoint)) web_point=\(point(NSPoint(x: webX, y: webY))) hit=\(describe(hit)) hit_local=\(point(hitLocal)) window=\(describe(window)) window_frame=\(rect(window.frame)) key_window=\(window.isKeyWindow ? 1 : 0) main_window=\(window.isMainWindow ? 1 : 0) app_key_window=\(describe(NSApp.keyWindow)) app_main_window=\(describe(NSApp.mainWindow)) backing_scale=\(window.screen?.backingScaleFactor ?? NSScreen.main?.backingScaleFactor ?? 1.0) web_view=\(describe(webView)) web_frame=\(rect(webView.frame)) web_bounds=\(rect(webView.bounds)) first_responder=\(describe(window.firstResponder)) responder_chain=\(responderChain(window.firstResponder)) target_nil=\(describe(targetNil)) target_webview=\(describe(targetWeb)) clipboard={\(clipboardSample())}")
        appendTrace("standalone-pdf-view-geometry-hit-chain label=\(label) chain=\(describe(hit)) point=\(point(hitLocal)) frame=\(rect(hit.frame)) bounds=\(rect(hit.bounds))")
        appendTrace("standalone-pdf-view-geometry-tree label=\(label) tree=\(viewTree(webView))")
        appendTrace("standalone-pdf-view-geometry-scroll label=\(label) scroll=\(scrollViews(webView))")
    }
    ...
    func checkTrigger() {
        ...
        } else if parts.first == "copy" {
            dump("before-copy", webX: 0, webY: 0)
            let ok = NSApp.sendAction(#selector(NSText.copy(_:)), to: nil, from: nil)
            appendTrace("standalone-pdf-view-geometry-copy ok=\(ok ? 1 : 0) clipboard={\(clipboardSample())}")
            dump("after-copy", webX: 0, webY: 0)
        }
...
run_standalone() {
  local ready window_id win_line sx sy ex ey before_change after_copy_text after_file after_len after_hash sample

  write_swift_app
  "$PROBE_APP" "$PDF_PATH" "$STANDALONE_TRACE" "$TRIGGER" "$ACK" >"$STANDALONE_STDOUT" 2>"$STANDALONE_STDERR" &
  PROBE_PID="$!"
  for _ in $(seq 1 80); do
    ready="$(grep -E "^READY window_id=[0-9]+" "$STANDALONE_STDOUT" | tail -1 || true)"
    [ -n "$ready" ] && break
    delay 0.25
  done
  [ -n "${ready:-}" ] || fail "standalone WKWebView did not become ready"
  window_id="$(printf '%s\n' "$ready" | sed -E 's/.*window_id=([0-9]+).*/\1/')"
  win_line="$(exact_window_bounds "$window_id")" || fail "standalone window bounds missing"
  read -r sx sy <<<"$(point_from_window_ratio "$win_line" "$START_X_RATIO" "$DRAG_Y_RATIO")"
  read -r ex ey <<<"$(point_from_window_ratio "$win_line" "$END_X_RATIO" "$DRAG_Y_RATIO")"

  activate_pid "$PROBE_PID" || true
  delay 0.5
  before_change="$(pasteboard_change_count)"
  printf '%s' "ISSUE834_EXP53_STANDALONE_SENTINEL_$RUN_ID" | pbcopy
  write_trigger dump before-drag "$((900 * 58 / 100))" "$((650 * 43 / 100))"
  swift "$ROOT/scripts/ghostty-app/inject.swift" drag "$sx" "$sy" "$ex" "$ey" >>"$HARNESS_LOG" 2>&1
  delay 1
  write_trigger dump after-drag "$((900 * 99 / 100))" "$((650 * 43 / 100))"
  write_trigger copy
  delay 1
  after_copy_text="$(pbpaste)"
  after_file="$RUN_DIR/standalone-after-copy.txt"
  printf '%s' "$after_copy_text" >"$after_file"
  after_len="$(wc -c <"$after_file" | tr -d ' ')"
  after_hash="$(hash_file "$after_file")"
  sample="$(printf '%s' "$after_copy_text" | head -c 160 | tr '\n' ' ')"

  python3 - "$LOG_DIR/standalone-wkpdf-summary-$RUN_ID.json" "$STANDALONE_TRACE" "$win_line" "$sx" "$sy" "$ex" "$ey" "$before_change" "$(pasteboard_change_count)" "$after_len" "$after_hash" "$sample" <<'PY'
import json
import sys
from pathlib import Path

target, trace, win_line, sx, sy, ex, ey, before, after, length, digest, sample = sys.argv[1:13]
tokens = ["LEFT834", "MID834", "RIGHT834"]
data = {
    "window_bounds": win_line,
    "drag_ratios": {"start_x": 0.58, "end_x": 0.99, "y": 0.43},
    "drag_global": {"start": [int(sx), int(sy)], "end": [int(ex), int(ey)]},
    "drag_web_points": {"start": [522, 279], "end": [891, 279]},
    "clipboard": {
...
print(json.dumps(data["clipboard"], indent=2, sort_keys=True))
PY
  kill "$PROBE_PID" >/dev/null 2>&1 || true
  PROBE_PID=""
}
...
run_embedded() {
  ...
  if TERMSURF_ISSUE834_PDF_FIXTURE_MODE=separated-tokens \
    ...
    TERMSURF_ISSUE834_PDF_COPY_DRAG_Y_RATIO="$DRAG_Y_RATIO" \
...
classify() {
  python3 - "$SUMMARY" "$RUN_ID" "$ORIGINAL_RESTORE_STATUS" "$ORACLE_SUMMARY" "$LOG_DIR/standalone-wkpdf-summary-$RUN_ID.json" "$EMBEDDED_SUMMARY" "$STANDALONE_TRACE" "$EMBEDDED_TRACE" "$EMBEDDED_COPY_TRACE" "$HARNESS_LOG" "$FIXTURE_IDENTITY" <<'PY'
import json
import re
import sys
from pathlib import Path
...
standalone_summary_path = Path(sys.argv[5])
embedded_summary_path = Path(sys.argv[6])
standalone_trace_path = Path(sys.argv[7])
embedded_trace_path = Path(sys.argv[8])
embedded_copy_trace_path = Path(sys.argv[9])
harness_log = Path(sys.argv[10])
fixture_identity = json.loads(sys.argv[11])
...
def hit_for(trace, prefix, label):
    pattern = rf"{re.escape(prefix)}.*label={re.escape(label)} .*hit=([^ ]+)"
    match = re.search(pattern, trace)
    return match.group(1) if match else ""

oracle = load_json(oracle_path)
standalone = load_json(standalone_summary_path)
embedded = load_json(embedded_summary_path)
standalone_trace = read_text(standalone_trace_path)
embedded_trace = read_text(embedded_trace_path)
embedded_copy_trace = read_text(embedded_copy_trace_path)
...
standalone_contains_all = bool(standalone and standalone.get("clipboard", {}).get("contains_all_tokens"))
...
trace_requirements = {
    "standalone_state": "standalone-pdf-view-geometry-state" in standalone_trace,
    "standalone_tree": "standalone-pdf-view-geometry-tree" in standalone_trace,
    "standalone_hit": "standalone-pdf-view-geometry-hit-chain" in standalone_trace,
    "standalone_scroll": "standalone-pdf-view-geometry-scroll" in standalone_trace,
    "embedded_state": "surfari-pdf-view-geometry-state" in embedded_trace,
    "embedded_tree": "surfari-pdf-view-geometry-tree" in embedded_trace,
    "embedded_hit": "surfari-pdf-view-geometry-hit-chain" in embedded_trace,
    "embedded_scroll": "surfari-pdf-view-geometry-scroll" in embedded_trace,
    "embedded_copy_state": "before-external-copy" in embedded_trace and "after-direct-copy" in embedded_trace,
}
traces_complete = all(trace_requirements.values())
...
embedded_coords = (embedded or {}).get("coordinate_mapping", {})
gesture_equivalence = bool(
    fixture_identity_match
    and standalone_contains_all
    and standalone
    and standalone.get("drag_ratios") == {"start_x": 0.58, "end_x": 0.99, "y": 0.43}
    and abs(float(embedded_coords.get("web_drag_start", {}).get("x", 0)) - float(embedded_coords.get("web_drag_end", {}).get("x", 0))) > 300
)

standalone_hit = hit_for(standalone_trace, "standalone-pdf-view-geometry-state", "after-drag")
embedded_hit = hit_for(embedded_trace, "surfari-pdf-view-geometry-state", "mouse-up")

if restore_status != "restored":
    result = "fail"
    classification = "clipboard-restore-failed"
elif not oracle_gate_open or not standalone_contains_all or not embedded_reproduced:
    result = "partial"
    classification = "harness-insufficient"
elif not traces_complete or not fixture_identity_match or not gesture_equivalence:
    result = "partial"
    classification = "harness-insufficient"
elif standalone_hit and embedded_hit and standalone_hit.split(":")[0] != embedded_hit.split(":")[0]:
    result = "pass"
    classification = "hit-test-target-gap"
else:
    result = "pass"
    classification = "geometry-equivalent"
...
data = {
    ...
    "standalone_summary": str(standalone_summary_path),
    "standalone_contains_all_tokens": standalone_contains_all,
