...
// Name match is a case-insensitive substring on purpose: // owner after the rename is "Roastty[DEBUG]". This harness passes the PID (from start-app.sh), so owner-name matching is bypassed. If two apps
