...
LOG_DIR="$ROOT/logs/issue-756-exp21-real-app-surfari-input-routing"
RUN_DIR="$(mktemp -d "${TMPDIR:-/tmp}/termsurf-issue756-exp21.XXXXXX")"
...
warn_if_file_pattern_after_missing() {
  local file="$1"
  local start_line="$2"
  local pattern="$3"
  local label="$4"
...
<html>
  <head>
    <meta charset="utf-8">
    <title>Issue 756 Surfari Input Initial</title>
    <style>
      html,
      body {
        margin: 0;
    ...
    <script>
      ...
      function report(kind, detail = "") {
        seq += 1;
        const marker = `ISSUE756_EXP21_INPUT seq=${seq} kind=${kind} ${detail}`.trim();
        console.log(marker);
        document.title = `Issue 756 Surfari Input ${kind} ${seq}`;
        ...
        input.addEventListener("keyup", (event) => {
          report("keyup", `key=${safe(event.key)} value=${safe(input.value)} active=${safe(document.activeElement.id)}`);
        });
        clickZone.addEventListener("click", (event) => {
          report("click", `detail=${event.detail} active=${safe(document.activeElement.id)}`);
        });
  ...
  <body>
    <main>
      <h1>ISSUE756_EXP21_SURFARI_INPUT</h1>
      <input id="field" autocomplete="off" spellcheck="false">
      <div id="click-zone">ISSUE756_EXP21_CLICK_ZONE</div>
      <div id="result">ISSUE756_EXP21_BOOT</div>
    </main>
  </body>
</html>
EOF
...
TERMSURF_GEOMETRY_SCENARIO=issue756-exp21-real-app-surfari-input-routing \
...
wait_for_file_pattern "$WEBTUI_TRACE" "event=console_message.*message=ISSUE756_EXP21_INPUT .*kind=ready" "fixture ready console marker"
wait_for_file_pattern "$WEBTUI_TRACE" "event=title_changed.*title=Issue 756 Surfari Input" "fixture title reached WebTUI"
...
IFS=$'\t' read -r _WID _WX _WY _WW _WH <<<"$WIN_LINE"
...
read -r CLICK_X CLICK_Y <<<"$(global_point_for_web_point "$WIN_LINE" "$PRESENTED_LINE" 160 170)"
...
wait_for_file_pattern_after "$WEBTUI_TRACE" "$KEY_STATE_START" "event=console_message.*message=ISSUE756_EXP21_INPUT .*kind=input .*value=${TOKEN} .*active=field" "page received typed token"
...
warn_if_file_pattern_after_missing "$WEBTUI_TRACE" "$CLICK_STATE_START" "event=console_message.*message=ISSUE756_EXP21_INPUT .*kind=click .*detail=1" "page observed click-zone click"
...
wait_for_file_pattern_after "$WEBTUI_TRACE" "$SCROLL_STATE_START" "event=console_message.*message=ISSUE756_EXP21_INPUT .*kind=wheel" "page observed wheel input"
