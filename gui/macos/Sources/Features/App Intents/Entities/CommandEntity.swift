...
struct CommandEntity: AppEntity {
    ...
    init(_ command: TermSurf.Command, for terminal: TerminalEntity) {
...
struct CommandQuery: EntityQuery {
    ...
    func entities(for identifiers: [CommandEntity.ID]) async throws -> [CommandEntity] {
        ...
        let commands = appDelegate.termsurf.config.commandPaletteEntries
    ...
    func suggestedEntities() async throws -> [CommandEntity] {
        ...
        return appDelegate.termsurf.config.commandPaletteEntries.map { CommandEntity($0, for: terminal) }
    }
}
