...
extension NSEvent {
    /// Create a TermSurf key event for a given keyboard action.
    ///
    ...
    func termsurfKeyEvent(
        ...
        key_ev.mods = TermSurf.termsurfMods(modifierFlags)
        key_ev.consumed_mods = TermSurf.termsurfMods(
    ...
    /// Returns the text to set for a key event for TermSurf.
    ///
    /// This namely contains logic to avoid control characters, since we handle control character
    /// mapping manually within TermSurf.
    var termsurfCharacters: String? {
        ...
        if characters.count == 1,
           let scalar = characters.unicodeScalars.first {
            ...
            // encoding directly within TermSurf's KeyEncoder.
            if scalar.value < 0x20 {
                return self.characters(byApplyingModifiers: modifierFlags.subtracting(.control))
            }

            // If we have a single value in the PUA, then it's a function key and
            // we don't want to send PUA ranges down to TermSurf.
