...
extension TermSurf {
    ...
    class SurfaceView: OSView, ObservableObject, Codable, Identifiable {
                ...
                if let searchState {
                    ...
                    searchNeedleCancellable = searchState.$needle
                        ...
                        .sink { [weak self] needle in
                            ...
                            termsurf_surface_binding_action(surface, action, UInt(action.lengthOfBytes(using: .utf8)))
                        }
        ...
        @Published var surfaceSize: termsurf_surface_size_s? = nil
        ...
        // detected with the set_password_input_cb on the TermSurf state.
        ...
        var needsConfirmQuit: Bool {
            ...
            return termsurf_surface_needs_confirm_quit(surface)
        }

        // Returns true if the process in this surface has exited.
        var processExited: Bool {
            guard let surface = self.surface else { return true }
            return termsurf_surface_process_exited(surface)
        }

        // Returns the inspector instance for this surface, or nil if the
        // surface has been closed or no inspector is active.
        var inspector: TermSurf.Inspector? {
            guard let surface = self.surface else { return nil }
            guard let cInspector = termsurf_surface_inspector(surface) else { return nil }
            return TermSurf.Inspector(cInspector: cInspector)
        }
        ...
        @Published var inspectorVisible: Bool = false {
            didSet {
                if (oldValue && !inspectorVisible) {
                    guard let surface = self.surface else { return }
                    termsurf_inspector_free(surface)
                }
            }
        }
        ...
        private(set) var surfaceModel: TermSurf.Surface?

        /// Returns the underlying C value for the surface. See "note" on surfaceModel.
        var surface: termsurf_surface_t? {
        ...
        var scrollbar: TermSurf.Action.Scrollbar?
        ...
        init(_ app: termsurf_app_t, baseConfig: SurfaceConfiguration? = nil, uuid: UUID? = nil) {
            self.markedText = NSMutableAttributedString()
            self.id = uuid ?? .init()

            // Our initial config always is our application wide config.
            if let appDelegate = NSApplication.shared.delegate as? AppDelegate {
                self.derivedConfig = DerivedConfig(appDelegate.termsurf.config)
            } else {
            ...
            cachedScreenContents = .init(duration: .milliseconds(500)) { [weak self] in
                ...
                guard let surface = self.surface else { return "" }
                var text = termsurf_text_s()
                let sel = termsurf_selection_s(
                    top_left: termsurf_point_s(
                        tag: TERMSURF_POINT_SCREEN,
                        coord: TERMSURF_POINT_COORD_TOP_LEFT,
                        x: 0,
                        y: 0),
                    bottom_right: termsurf_point_s(
                        tag: TERMSURF_POINT_SCREEN,
                        coord: TERMSURF_POINT_COORD_BOTTOM_RIGHT,
                        x: 0,
                        y: 0),
                    rectangle: false)
                guard termsurf_surface_read_text(surface, sel, &text) else { return "" }
                defer { termsurf_surface_free_text(surface, &text) }
            ...
            center.addObserver(
                ...
                name: TermSurf.Notification.didUpdateRendererHealth,
                object: self)
            center.addObserver(
                self,
                selector: #selector(termsurfDidContinueKeySequence),
                name: TermSurf.Notification.didContinueKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(termsurfDidEndKeySequence),
                name: TermSurf.Notification.didEndKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(termsurfDidChangeKeyTable),
                name: TermSurf.Notification.didChangeKeyTable,
                object: self)
            center.addObserver(
                self,
            ...
            center.addObserver(
                self,
                selector: #selector(termsurfColorDidChange(_:)),
            ...
            center.addObserver(
                self,
                selector: #selector(termsurfDidChangeReadonly(_:)),
            ...
            let surface = surface_cfg.withCValue(view: self) { surface_cfg_c in
                termsurf_surface_new(app, &surface_cfg_c)
            }
            guard let surface = surface else {
                self.error = TermSurf.Error.apiFailed
                return
            }
            self.surfaceModel = TermSurf.Surface(cSurface: surface)
        ...
        func focusDidChange(_ focused: Bool) {
            ...
            termsurf_surface_set_focus(surface, focused)
            termsurf_surface_pane_focus_changed(surface, focused)  // Issue 606
        ...
        func sizeDidChange(_ size: CGSize) {
            // TermSurf wants to know the actual framebuffer size... It is very important
        ...
        private func setSurfaceSize(width: UInt32, height: UInt32) {
            ...
            termsurf_surface_set_size(surface, width, height)

            // Update our cached size metrics
            let size = termsurf_surface_size(surface)
        ...
        func setCursorShape(_ shape: termsurf_action_mouse_shape_e) {
            switch (shape) {
            case TERMSURF_MOUSE_SHAPE_DEFAULT:
                pointerStyle = .default

            case TERMSURF_MOUSE_SHAPE_TEXT:
                pointerStyle = .horizontalText

            case TERMSURF_MOUSE_SHAPE_GRAB:
                pointerStyle = .grabIdle

            case TERMSURF_MOUSE_SHAPE_GRABBING:
                pointerStyle = .grabActive

            case TERMSURF_MOUSE_SHAPE_POINTER:
                pointerStyle = .link

            case TERMSURF_MOUSE_SHAPE_W_RESIZE:
                pointerStyle = .resizeLeft

            case TERMSURF_MOUSE_SHAPE_E_RESIZE:
                pointerStyle = .resizeRight

            case TERMSURF_MOUSE_SHAPE_N_RESIZE:
                pointerStyle = .resizeUp

            case TERMSURF_MOUSE_SHAPE_S_RESIZE:
                pointerStyle = .resizeDown

            case TERMSURF_MOUSE_SHAPE_NS_RESIZE:
                pointerStyle = .resizeUpDown

            case TERMSURF_MOUSE_SHAPE_EW_RESIZE:
                pointerStyle = .resizeLeftRight

            case TERMSURF_MOUSE_SHAPE_VERTICAL_TEXT:
                pointerStyle = .verticalText

            case TERMSURF_MOUSE_SHAPE_CONTEXT_MENU:
                pointerStyle = .contextMenu

            case TERMSURF_MOUSE_SHAPE_CROSSHAIR:
                pointerStyle = .crosshair

            case TERMSURF_MOUSE_SHAPE_NOT_ALLOWED:
        ...
        @objc private func onUpdateRendererHealth(notification: SwiftUI.Notification) {
            ...
            guard let health = healthAny as? termsurf_action_renderer_health_e else { return }
            DispatchQueue.main.async { [weak self] in
                self?.healthy = health == TERMSURF_RENDERER_HEALTH_OK
            }
        }

        @objc private func termsurfDidContinueKeySequence(notification: SwiftUI.Notification) {
            guard let keyAny = notification.userInfo?[TermSurf.Notification.KeySequenceKey] else { return }
        ...
        @objc private func termsurfDidEndKeySequence(notification: SwiftUI.Notification) {
            DispatchQueue.main.async { [weak self] in
                self?.keySequence = []
            }
        }

        @objc private func termsurfDidChangeKeyTable(notification: SwiftUI.Notification) {
            guard let action = notification.userInfo?[TermSurf.Notification.KeyTableKey] as? TermSurf.Action.KeyTable else { return }
        ...
        @objc private func termsurfConfigDidChange(_ notification: SwiftUI.Notification) {
            // Get our managed configuration object out
            guard let config = notification.userInfo?[
                SwiftUI.Notification.Name.TermSurfConfigChangeKey
        ...
        @objc private func termsurfColorDidChange(_ notification: SwiftUI.Notification) {
            guard let change = notification.userInfo?[
                SwiftUI.Notification.Name.TermSurfColorChangeKey
            ] as? TermSurf.Action.ColorChange else { return }
        ...
        @objc private func termsurfBellDidRing(_ notification: SwiftUI.Notification) {
            // Bell state goes to true
            bell = true
        }

        @objc private func termsurfDidChangeReadonly(_ notification: SwiftUI.Notification) {
        ...
        @objc private func windowDidChangeScreen(notification: SwiftUI.Notification) {
            ...
            // When the window changes screens, we need to update libtermsurf with the screen
            // ID. If vsync is enabled, this will be used with the CVDisplayLink to ensure
            // the proper refresh rate is going.
            termsurf_surface_set_display_id(surface, screen.displayID ?? 0)
        ...
        override func viewDidChangeBackingProperties() {
            ...
            termsurf_surface_set_content_scale(surface, xScale, yScale)
        ...
        override func mouseDown(with event: NSEvent) {
            guard let surface = self.surface else { return }
            let mods = TermSurf.termsurfMods(event.modifierFlags)
            termsurf_surface_mouse_button(surface, TERMSURF_MOUSE_PRESS, TERMSURF_MOUSE_LEFT, mods)
        }
        ...
        override func mouseUp(with event: NSEvent) {
            ...
            guard let surface = self.surface else { return }
            let mods = TermSurf.termsurfMods(event.modifierFlags)
            termsurf_surface_mouse_button(surface, TERMSURF_MOUSE_RELEASE, TERMSURF_MOUSE_LEFT, mods)

            // Release pressure
            termsurf_surface_mouse_pressure(surface, 0, 0)
        }

        override func otherMouseDown(with event: NSEvent) {
            guard let surface = self.surface else { return }
            let mods = TermSurf.termsurfMods(event.modifierFlags)
            let button = TermSurf.Input.MouseButton(fromNSEventButtonNumber: event.buttonNumber)
            termsurf_surface_mouse_button(surface, TERMSURF_MOUSE_PRESS, button.cMouseButton, mods)
        }
        ...
        override func otherMouseUp(with event: NSEvent) {
            ...
            let mods = TermSurf.termsurfMods(event.modifierFlags)
            let button = TermSurf.Input.MouseButton(fromNSEventButtonNumber: event.buttonNumber)
            termsurf_surface_mouse_button(surface, TERMSURF_MOUSE_RELEASE, button.cMouseButton, mods)
        }
        ...
        override func rightMouseDown(with event: NSEvent) {
            ...
            let mods = TermSurf.termsurfMods(event.modifierFlags)
            if (termsurf_surface_mouse_button(
                surface,
        ...
        override func mouseEntered(with event: NSEvent) {
            ...
            let mouseEvent = TermSurf.Input.MousePosEvent(
                x: pos.x,
        ...
        override func scrollWheel(with event: NSEvent) {
            ...
            let scrollMods = TermSurf.Input.ScrollMods(
                precision: precision,
        ...
        override func pressureChange(with event: NSEvent) {
            ...
            // Notify TermSurf first. We do this because this will let TermSurf handle
            // state setup that we'll need for later pressure handling (such as
            // QuickLook)
            termsurf_surface_mouse_pressure(surface, UInt32(event.stage), Double(event.pressure))
        ...
        override func keyDown(with event: NSEvent) {
            ...
            let translationModsTermSurf = TermSurf.eventModifierFlags(
                mods: termsurf_surface_key_translation_mods(
                    surface,
                    TermSurf.termsurfMods(event.modifierFlags)
                )
            )

            // There are hidden bits set in our event that matter for certain dead keys
            // so we can't use translationModsTermSurf directly. Instead, we just check
            // for exact states and set them.
            var translationMods = event.modifierFlags
            for flag in [NSEvent.ModifierFlags.shift, .control, .option, .command] {
                if (translationModsTermSurf.contains(flag)) {
            ...
            let action = event.isARepeat ? TERMSURF_ACTION_REPEAT : TERMSURF_ACTION_PRESS
            ...
            if let list = keyTextAccumulator, list.count > 0 {
                ...
                _ = keyAction(
                    ...
                    event: event,
                    translationEvent: translationEvent,
                    text: translationEvent.termsurfCharacters,
        ...
        override func keyUp(with event: NSEvent) {
            _ = keyAction(TERMSURF_ACTION_RELEASE, event: event)
        }
        ...
        override func performKeyEquivalent(with event: NSEvent) -> Bool {
            ...
            // TermSurf bindings or the macOS menu system (Issue 609 Experiment 2).
            ...
            let bindingFlags = surfaceModel.flatMap { surface in
                ...
                return (event.characters ?? "").withCString { ptr in
                    ...
                    return surface.keyIsBinding(termsurfEvent)
                }
            }
        ...
        override func flagsChanged(with event: NSEvent) {
            ...
            case 0x39: mod = TERMSURF_MODS_CAPS.rawValue
            case 0x38, 0x3C: mod = TERMSURF_MODS_SHIFT.rawValue
            case 0x3B, 0x3E: mod = TERMSURF_MODS_CTRL.rawValue
            case 0x3A, 0x3D: mod = TERMSURF_MODS_ALT.rawValue
            case 0x37, 0x36: mod = TERMSURF_MODS_SUPER.rawValue
            default: return
            }
            ...
            var action = TERMSURF_ACTION_RELEASE
        ...
        private func keyAction(
            ...
            var key_ev = event.termsurfKeyEvent(action, translationMods: translationEvent?.modifierFlags)
            key_ev.composing = composing

            // For text, we only encode UTF8 if we don't have a single control
            // character. Control characters are encoded by TermSurf itself.
            // Without this, `ctrl+enter` does the wrong thing.
            if let text, text.count > 0,
               let codepoint = text.utf8.first, codepoint >= 0x20 {
                return text.withCString { ptr in
                    key_ev.text = ptr
                    return termsurf_surface_key(surface, key_ev)
                }
            } else {
                return termsurf_surface_key(surface, key_ev)
            }
        ...
        override func quickLook(with event: NSEvent) {
            ...
            var text = termsurf_text_s()
            guard termsurf_surface_quicklook_word(surface, &text) else { return super.quickLook(with: event) }
            defer { termsurf_surface_free_text(surface, &text) }
            ...
            if let fontRaw = termsurf_surface_quicklook_font(surface) {
                // Memory management here is wonky: termsurf_surface_quicklook_font
            ...
            // TermSurf coordinate system is top-left, convert to bottom-left for AppKit
        ...
        @IBAction func splitRight(_ sender: Any) {
            guard let surface = self.surface else { return }
            termsurf_surface_split(surface, TERMSURF_SPLIT_DIRECTION_RIGHT)
        }

        @IBAction func splitLeft(_ sender: Any) {
            guard let surface = self.surface else { return }
            termsurf_surface_split(surface, TERMSURF_SPLIT_DIRECTION_LEFT)
        }

        @IBAction func splitDown(_ sender: Any) {
            guard let surface = self.surface else { return }
            termsurf_surface_split(surface, TERMSURF_SPLIT_DIRECTION_DOWN)
        }

        @IBAction func splitUp(_ sender: Any) {
            guard let surface = self.surface else { return }
            termsurf_surface_split(surface, TERMSURF_SPLIT_DIRECTION_UP)
        }
        ...
        func showUserNotification(title: String, body: String) {
            ...
            content.categoryIdentifier = TermSurf.userNotificationCategory
        ...
        struct DerivedConfig {
            ...
            let scrollbar: TermSurf.Config.Scrollbar
            ...
            init(_ config: TermSurf.Config) {
                ...
                self.windowAppearance = .init(termsurfConfig: config)
...
extension TermSurf.SurfaceView: NSTextInputClient {
    ...
    func selectedRange() -> NSRange {
        ...
        // Get our range from the TermSurf API. There is a race condition between getting the
        ...
        var text = termsurf_text_s()
        guard termsurf_surface_read_selection(surface, &text) else { return NSRange() }
        defer { termsurf_surface_free_text(surface, &text) }
    ...
    func attributedSubstring(forProposedRange range: NSRange, actualRange: NSRangePointer?) -> NSAttributedString? {
        // TermSurf.logger.warning("pressure substring range=\(range) selectedRange=\(self.selectedRange())")
        guard let surface = self.surface else { return nil }
        ...
        var text = termsurf_text_s()
        guard termsurf_surface_read_selection(surface, &text) else { return nil }
        defer { termsurf_surface_free_text(surface, &text) }
    ...
    func firstRect(forCharacterRange range: NSRange, actualRange: NSRangePointer?) -> NSRect {
        ...
        // TermSurf will tell us where it thinks an IME keyboard should render.
        var x: Double = 0
        var y: Double = 0
        ...
        if range.length > 0 && range != self.selectedRange() {
            // QuickLook
            var text = termsurf_text_s()
            if termsurf_surface_read_selection(surface, &text) {
                ...
                // Free our text
                termsurf_surface_free_text(surface, &text)
            } else {
                termsurf_surface_ime_point(surface, &x, &y, &width, &height)
            }
        } else {
            termsurf_surface_ime_point(surface, &x, &y, &width, &height)
        }
        ...
        // TermSurf coordinates are in top-left (0, 0) so we have to convert to
    ...
    /// Sync the preedit state based on the markedText value to libtermsurf
...
extension TermSurf.SurfaceView: NSServicesMenuRequestor {
    override func validRequestor(
        ...
        if (returnType == nil || receivable.contains(returnType!)) &&
            ...
            // performance hits because `termsurf_surface_has_selection` isn't free.
            if let sendType, sendableRequiresSelection.contains(sendType) {
                if surface == nil || !termsurf_surface_has_selection(surface) {
    ...
    func writeSelection(
        ...
        // Read the selection
        var text = termsurf_text_s()
        guard termsurf_surface_read_selection(surface, &text) else { return false }
        defer { termsurf_surface_free_text(surface, &text) }
    ...
    func readSelection(from pboard: NSPasteboard) -> Bool {
        ...
        str.withCString { ptr in
            ...
            termsurf_surface_text(surface, ptr, UInt(len - 1))
        }
...
extension TermSurf.SurfaceView: NSMenuItemValidation {
    func validateMenuItem(_ item: NSMenuItem) -> Bool {
        switch item.action {
        case #selector(pasteSelection):
            let pb = NSPasteboard.termsurfSelection
...
extension TermSurf.SurfaceView {
    ...
    override func performDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        ...
        if let url = pb.string(forType: .URL) {
            ...
            content = TermSurf.Shell.escape(url)
           ...
           urls.count > 0 {
            ...
            content = urls
                .map { TermSurf.Shell.escape($0.path) }
                .joined(separator: " ")
...
extension TermSurf.SurfaceView {
    ...
    /// this to copy styling information as well but we need to augment TermSurf core to
    /// expose that.
    ///
