...
extension TermSurf {
    ...
    struct InspectableSurface: View {
        @EnvironmentObject var termsurf: TermSurf.App
        ...
        var body: some View {
            ...
            ZStack {
                if (!surfaceView.inspectorVisible) {
                    ...
                    SplitView(.vertical, $split, dividerColor: termsurf.config.splitDividerColor, left: {
                        SurfaceWrapper(surfaceView: surfaceView, isSplit: isSplit)
                    }, right: {
        ...
        private func onControlInspector(_ notification: SwiftUI.Notification) {
            ...
            guard let mode = modeAny as? termsurf_action_inspector_e else { return }

            switch (mode) {
            case TERMSURF_INSPECTOR_TOGGLE:
                surfaceView.inspectorVisible = !surfaceView.inspectorVisible

            case TERMSURF_INSPECTOR_SHOW:
                surfaceView.inspectorVisible = true

            case TERMSURF_INSPECTOR_HIDE:
    ...
    class InspectorView: MTKView, NSTextInputClient {
        ...
        private var inspector: TermSurf.Inspector? {
        ...
        override func mouseDown(with event: NSEvent) {
            ...
            inspector.mouseButton(TERMSURF_MOUSE_PRESS, button: TERMSURF_MOUSE_LEFT, mods: mods)
        }

        override func mouseUp(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = TermSurf.termsurfMods(event.modifierFlags)
            inspector.mouseButton(TERMSURF_MOUSE_RELEASE, button: TERMSURF_MOUSE_LEFT, mods: mods)
        }

        override func rightMouseDown(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = TermSurf.termsurfMods(event.modifierFlags)
            inspector.mouseButton(TERMSURF_MOUSE_PRESS, button: TERMSURF_MOUSE_RIGHT, mods: mods)
        }

        override func rightMouseUp(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = TermSurf.termsurfMods(event.modifierFlags)
            inspector.mouseButton(TERMSURF_MOUSE_RELEASE, button: TERMSURF_MOUSE_RIGHT, mods: mods)
        }
        ...
        override func scrollWheel(with event: NSEvent) {
            ...
            var momentum: termsurf_input_mouse_momentum_e = TERMSURF_MOUSE_MOMENTUM_NONE
            switch (event.momentumPhase) {
            case .began:
                momentum = TERMSURF_MOUSE_MOMENTUM_BEGAN
            case .stationary:
                momentum = TERMSURF_MOUSE_MOMENTUM_STATIONARY
            case .changed:
                momentum = TERMSURF_MOUSE_MOMENTUM_CHANGED
            case .ended:
                momentum = TERMSURF_MOUSE_MOMENTUM_ENDED
            case .cancelled:
                momentum = TERMSURF_MOUSE_MOMENTUM_CANCELLED
            case .mayBegin:
                momentum = TERMSURF_MOUSE_MOMENTUM_MAY_BEGIN
            default:
                break
            }
        ...
        override func keyUp(with event: NSEvent) {
            keyAction(TERMSURF_ACTION_RELEASE, event: event)
        }
        ...
        override func flagsChanged(with event: NSEvent) {
            ...
            if (mods.rawValue & mod != 0) { action = TERMSURF_ACTION_PRESS }

            keyAction(action, event: event)
        }

        private func keyAction(_ action: termsurf_input_action_e, event: NSEvent) {
            guard let inspector = self.inspector else { return }
            guard let key = TermSurf.Input.Key(keyCode: event.keyCode) else { return }
            let mods = TermSurf.termsurfMods(event.modifierFlags)
