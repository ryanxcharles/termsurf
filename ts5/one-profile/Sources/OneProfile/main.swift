...
func handleMessage(_ msg: xpc_object_t) {
    ...
    if actionStr == "display_surface" {
        ...
        guard port != MACH_PORT_NULL else {
            fputs("[OneProfile] null Mach port\n", stderr)
            return
        }

        guard let surface = IOSurfaceLookupFromMachPort(port) else {
            mach_port_deallocate(mach_task_self_, port)
            fputs("[OneProfile] IOSurfaceLookupFromMachPort failed\n", stderr)
            return
        }
        mach_port_deallocate(mach_task_self_, port)
        ...
        if elapsed >= 1.0 {
            ...
            fputs("[OneProfile] \(gFrameCount) frames (\(fpsStr) fps) | " +
                  "IOSurface \(w)x\(h)\n", stderr)
            gFrameCount = 0
            gLastLogTime = now
        }
...
func startXPCListener() {
    let queue = DispatchQueue(label: "com.termsurf.one-profile.xpc")
    let listener = xpc_connection_create_mach_service(
        "com.termsurf.one-profile",
        queue,
    ...
    xpc_connection_set_event_handler(listener) { peer in
        if xpc_get_type(peer) == XPC_TYPE_CONNECTION {
            ...
            fputs("[OneProfile] Profile server connected (\(gPeers.count) total)\n", stderr)
            ...
            xpc_connection_set_event_handler(peerConn) { event in
                if xpc_get_type(event) == XPC_TYPE_DICTIONARY {
                    ...
                    if event === XPC_ERROR_CONNECTION_INVALID {
                        ...
                        fputs("[OneProfile] XPC error\n", stderr)
                    }
                }
            }
            xpc_connection_resume(peerConn)
        } else if xpc_get_type(peer) == XPC_TYPE_ERROR {
            fputs("[OneProfile] Listener error\n", stderr)
        }
    }

    xpc_connection_resume(listener)
    fputs("[OneProfile] Listening on com.termsurf.one-profile...\n", stderr)
}

// MARK: - App delegate

class OneProfileAppDelegate: NSObject, NSApplicationDelegate {
    var window: NSWindow!
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        fputs("[OneProfile] Window and Metal pipeline ready\n", stderr)
    }
    ...
    func setupMetal(view: NSView) {
        ...
        do {
            ...
            fputs("[OneProfile] Loaded shaders from \(libPath)\n", stderr)
        } catch {
            fputs("[OneProfile] Failed to load \(libPath): \(error)\n", stderr)
            exit(1)
        }

        guard let vertexFunc = library.makeFunction(name: "vertex_main"),
              let fragmentFunc = library.makeFunction(name: "fragment_main") else {
            fputs("[OneProfile] Failed to find shader functions\n", stderr)
            exit(1)
        }
        ...
        do {
            ...
            fputs("[OneProfile] Failed to create pipeline: \(error)\n", stderr)
            exit(1)
        }
    ...
    @objc func render() {
        ...
        guard let tex = gCurrentTexture else { return }
...
let delegate = OneProfileAppDelegate()
