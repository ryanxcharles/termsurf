...
func handleMessage(_ msg: xpc_object_t) {
    ...
    if actionStr == "display_surface" {
        ...
        guard let surface = IOSurfaceLookupFromMachPort(port) else {
            ...
            fputs("[TwoProfiles] IOSurfaceLookupFromMachPort failed\n", stderr)
            return
        }
        mach_port_deallocate(mach_task_self_, port)
        ...
        if elapsed >= 1.0 {
            ...
            fputs("[TwoProfiles] L: \(gFrameCount[Pane.left.rawValue]) (\(fpsLStr) fps) " +
...
func startXPCListener() {
    let queue = DispatchQueue(label: "com.termsurf.two-profiles.xpc")
    ...
    xpc_connection_set_event_handler(listener) { peer in
        if xpc_get_type(peer) == XPC_TYPE_CONNECTION {
            ...
            fputs("[TwoProfiles] Profile server connected (\(gPeers.count) total)\n", stderr)
            ...
            xpc_connection_set_event_handler(peerConn) { event in
                if xpc_get_type(event) == XPC_TYPE_DICTIONARY {
                    ...
                    if event === XPC_ERROR_CONNECTION_INVALID {
                        ...
                        fputs("[TwoProfiles] XPC error\n", stderr)
                    }
                }
            }
            xpc_connection_resume(peerConn)
        } else if xpc_get_type(peer) == XPC_TYPE_ERROR {
            fputs("[TwoProfiles] Listener error\n", stderr)
        }
    }

    xpc_connection_resume(listener)
    fputs("[TwoProfiles] Listening on com.termsurf.two-profiles...\n", stderr)
}

// MARK: - App delegate

class TwoProfilesAppDelegate: NSObject, NSApplicationDelegate {
    var window: NSWindow!
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        fputs("[TwoProfiles] Window and Metal pipeline ready\n", stderr)
    }
    ...
    func setupMetal(view: NSView) {
        ...
        do {
            ...
            fputs("[TwoProfiles] Loaded shaders from \(libPath)\n", stderr)
        } catch {
            fputs("[TwoProfiles] Failed to load \(libPath): \(error)\n", stderr)
            exit(1)
        }

        guard let vertexFunc = library.makeFunction(name: "vertex_main"),
              let fragmentFunc = library.makeFunction(name: "fragment_main") else {
            fputs("[TwoProfiles] Failed to find shader functions\n", stderr)
            exit(1)
        }
        ...
        do {
            ...
            fputs("[TwoProfiles] Failed to create pipeline: \(error)\n", stderr)
            exit(1)
        }
...
let delegate = TwoProfilesAppDelegate()
